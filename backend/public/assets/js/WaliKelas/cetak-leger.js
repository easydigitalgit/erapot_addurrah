...
function loadLegerData() {
  ...
  fetch(`${baseUrl}wali/cetak-leger/get-data`, {
      method: 'POST',
      body: formData,
  ...
  .then(res => {
      if (res.status === 'success') {
          ...
          tbody.innerHTML = `<tr class="bg-white dark:bg-slate-800 transition-colors"><td colspan="${totalCols}" class="text-center py-12 text-red-500 font-medium text-sm border-b border-gray-200 dark:border-slate-700">${res.message || LANG.js_err_load}</td></tr>`;
          updateInfoKelas({
              nama_rombel: '-',
              wali_kelas: '-',
              jumlah_siswa: 0,
              kurikulum: '-',
              status_text: 'DATA KOSONG',
              status_color: 'gray'
          });
...
function updateStatisticsCards(dataSiswa) {
    ...
    document.getElementById('text_nilai_tertinggi').innerText = `${tertinggi.nama} - ${terringgi = tertinggi.mapel}`;
...
function exportExcel() {
    ...
    if (!rombel_id) {
        showToast('Data rombel tidak ditemukan.', 'error');
        return;
    }
    
    const baseUrl = window.BASE_URL || '/';
    const url = `${baseUrl}wali/cetak-leger/export-excel?tahun_ajaran=${encodeURIComponent(ta_id)}&semester=${encodeURIComponent(ta_semester)}&rombel_id=${rombel_id}&kategori=${encodeURIComponent(kategori_val)}`;

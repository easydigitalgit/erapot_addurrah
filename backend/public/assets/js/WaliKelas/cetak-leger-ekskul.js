...
async function loadLegerData() {
    ...
    if (!rombel_id) {
        body.innerHTML = `<tr><td colspan="${4 + EKSKUL_LIST.length}" class="text-center py-12 text-gray-400 font-medium italic">${window.LANG.no_data || 'Tidak ada data siswa ditemukan untuk rombel ini.'}</td></tr>`;
        foot.innerHTML = '';
        return;
    }
    ...
    try {
        ...
        const res = await fetch(window.BASE_URL + 'wali/cetak-leger-ekskul/get-data', {
            method: 'POST',
            body: formData,
        ...
        if (result.status === 'success') {
            ...
            body.innerHTML = `<tr><td colspan="${4 + EKSKUL_LIST.length}" class="text-center py-12 text-rose-500 font-medium border-b border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-800">${result.message || window.LANG.no_data}</td></tr>`;
            foot.innerHTML = '';
            document.getElementById('total_peserta_count').textContent = '0';
        }
    } catch (e) {
...
window.exportToExcel = function(event) {
    ...
    if (!rombel_id) {
        alert('Data rombel tidak ditemukan.');
        return;
    }
    ...
    setTimeout(() => {
        window.location.href = window.BASE_URL + 'wali/cetak-leger-ekskul/export-excel?ta_smt=' + encodeURIComponent(taSmt);

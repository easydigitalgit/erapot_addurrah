...
function applyFilters() {
  ...
  filteredStudents = students.filter((s) => {
    ...
    // Gunakan status_aman yang sudah dinormalisasi saat load
    const statusSiswa = String(s.status_aman || "Aktif").trim();
    ...
    if (status === "") {
        matchStatus = true; // "Semua Status"
    } else if (status === "Nonaktif") {
        matchStatus = (statusSiswa === "Pindah" || statusSiswa === "Keluar");
    } else {
        matchStatus = (statusSiswa === status); // Cocokkan langsung (Aktif = Aktif, Lulus = Lulus)
    }
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  ["searchInput", "filterLevel", "filterStatus", "filterTahunAkurat"].forEach(
    (id) => {
      const el = document.getElementById(id);
      if (el) {
        el.addEventListener(
          id === "searchInput" ? "keyup" : "change",
          applyFilters,
        );
      }
    },
  ...
  // =====================================================================
  // DETEKSI PERUBAHAN TANGGAL UNTUK MERUBAH NIS (Hanya saat Tambah Data)
  // =====================================================================
  const tglDiterimaInput = document.getElementById("tgl_diterima");
  if (tglDiterimaInput) {
    tglDiterimaInput.addEventListener("change", function () {
      // Hanya jalan jika form sedang mode "Tambah" (ID masih kosong)
      if (document.querySelector('input[name="id"]').value === "") {
        const nisInput = document.getElementById("nis");
        if (nisInput) nisInput.value = "Menghitung ulang NIS...";

        const baseUrl = document.getElementById("base-url").dataset.url;
        fetch(
          `${baseUrl}/admin/siswa/generateNextNis?tgl_diterima=${this.value}`,
        )
          .then((res) => res.json())
          .then((data) => {
            if (data.status === "success" && nisInput) {
...
// LOGIKA BADGE STATUS (Warna Dinamis Sesuai Status)
      let badgeClass = "";
      if (status === "Aktif") {
          badgeClass = "bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-800/50";
      } else if (status === "Lulus") {
          badgeClass = "bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-400 border border-purple-200 dark:border-purple-800/50";
      } else if (status === "Pindah") {
          badgeClass = "bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400 border border-amber-200 dark:border-amber-800/50";
      } else if (status === "Keluar") {
          badgeClass = "bg-rose-100 text-rose-700 dark:bg-rose-900/30 dark:text-rose-400 border border-rose-200 dark:border-rose-800/50";
      } else {
          badgeClass = "bg-gray-100 text-gray-700 dark:bg-slate-700 dark:text-slate-300 border border-gray-200 dark:border-slate-600";
      }

      // Text status terjemahan
      let statusTextUI = status;
      if (status === "Aktif") statusTextUI = window.LANG?.active || "Aktif";
      else if (status === "Lulus") statusTextUI = window.LANG?.graduated || "Lulus";
      else if (status === "Pindah") statusTextUI = window.LANG?.moved || "Pindah";
      else if (status === "Keluar") statusTextUI = window.LANG?.dropped_out || "Keluar";

      // LOGIKA HYBRID (Cari di foto_profil dulu, baru foto_siswa)
      let finalFoto = student.foto_profil;
      if (!finalFoto || finalFoto === "null" || finalFoto === "") {
        finalFoto = student.foto_siswa;
      }
      ...
      if (
        finalFoto &&
        finalFoto !== "null" &&
        String(finalFoto).trim() !== ""
      ) {
        const cleanBaseUrl = baseUrl.replace(/\/$/, "");
        const cacheBuster = "?v=" + new Date().getTime();
        const fotoUrl = `${cleanBaseUrl}/assets/uploads/avatars/${finalFoto}${cacheBuster}`;
        // onerror: Jika di folder avatars tidak ada, cari di folder siswa lama
        avatarHTML = `<img src="${fotoUrl}" class="w-full h-full object-cover border border-slate-200 dark:border-slate-600 shadow-sm" onerror="this.onerror=null; this.src='${cleanBaseUrl}/uploads/siswa/${finalFoto}';">`;
      } else {
...
window.showAddModal = function () {
  ...
  const baseUrl = document.getElementById("base-url").dataset.url;
  const nisInput = document.getElementById("nis"); // Tangkap kotak NIS
  ...
  if (subtitleEl)
    subtitleEl.innerText =
      LANG.form_subtitle || "Input data siswa beserta wali secara bersamaan";
  if (btnSubmit)
    btnSubmit.innerHTML = `<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" /></svg> ${LANG.save_all_data || "Simpan Seluruh Data"}`;

  // ==========================================
  // FITUR BARU: AUTO LOAD PREDIKSI NIS
  // ==========================================
  if (nisInput) {
    nisInput.value = "Menghitung NIS..."; // Teks loading

    fetch(`${baseUrl}/admin/siswa/generateNextNis`)
      .then((res) => res.json())
      .then((data) => {
        if (data.status === "success") {
          nisInput.value = data.nis; // Masukkan hasil NIS dari database
        }
      })
      .catch((err) => {
        console.error("Gagal mengambil NIS:", err);
        nisInput.value = "Gagal memuat";
      });
  }
  // ==========================================
...
window.editStudent = async function (id) {
  ...
  if (subtitleEl)
    subtitleEl.innerText =
      LANG.form_subtitle || "Update informasi siswa secara detail";
  if (btnSubmit)
    btnSubmit.innerHTML = `<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" /></svg> ${LANG.btn_update || "Update Data"}`;
  ...
  setVal("ekskul_3", student.ekskul_3);

  // UBAH BAGIAN PHOTO PREVIEW EDIT
  let finalFotoEdit = student.foto_profil;
  if (!finalFotoEdit || finalFotoEdit === "null" || finalFotoEdit === "")
    finalFotoEdit = student.foto_siswa;

  const preview = document.getElementById("photoPreview");
  if (
    finalFotoEdit &&
    finalFotoEdit !== "null" &&
    String(finalFotoEdit).trim() !== ""
  ) {
    const cleanBaseUrl = baseUrl.replace(/\/$/, "");
    const cacheBuster = "?v=" + new Date().getTime();
    preview.innerHTML = `<img src="${cleanBaseUrl}/assets/uploads/avatars/${finalFotoEdit}${cacheBuster}" class="w-full h-full object-cover" onerror="this.onerror=null; this.src='${cleanBaseUrl}/uploads/siswa/${finalFotoEdit}';">`;
  } else {
    ...
    if (result.isConfirmed) {
      ...
      fetch(url, {
        ...
        .catch((err) => {
          ...
          Swal.fire(
            "Error!",
            "Terjadi kesalahan. Jika terus berlanjut, muat ulang (refresh) halaman ini.",
            "error",
          );
        });
    }
...
window.showDetailDrawer = function (id) {
  ...
  const v = (val) =>
    val && String(val).trim() !== "" && val !== "null" ? val : "-";

  const fmtDate = (dateStr) => {
    if (!dateStr || dateStr === "null" || dateStr === "0000-00-00") return "-";
    const date = new Date(dateStr);
    return new Intl.DateTimeFormat(APP_LANG, {
      day: "numeric",
      month: "long",
      year: "numeric",
    }).format(date);
  ...
  document.getElementById("detailGender").innerText =
    s.jk_aman === "L" ? "Laki-laki" : s.jk_aman === "P" ? "Perempuan" : "-";
  ...
  // UBAH BAGIAN FOTO DI DRAWER/LACI
  let finalFotoDrawer = s.foto_profil;
  if (!finalFotoDrawer || finalFotoDrawer === "null" || finalFotoDrawer === "")
    finalFotoDrawer = s.foto_siswa;

  if (
    finalFotoDrawer &&
    finalFotoDrawer !== "null" &&
    String(finalFotoDrawer).trim() !== ""
  ) {
    const cleanBaseUrl = baseUrl.replace(/\/$/, "");
    const cacheBuster = "?v=" + new Date().getTime();
    const fotoUrl = `${cleanBaseUrl}/assets/uploads/avatars/${finalFotoDrawer}${cacheBuster}`;
    avatarContainer.innerHTML = `<img src="${fotoUrl}" class="w-full h-full object-cover" onerror="this.onerror=null; this.src='${cleanBaseUrl}/uploads/siswa/${finalFotoDrawer}';">`;
  } else {
  ...
  document.getElementById("detailHp").innerHTML =
  ...
  document.getElementById("detailEmail").innerHTML =
    `<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/></svg> ${v(s.email_siswa)}`;

  let rombel = "-";
  if (s.nama_rombel) rombel = `${s.tingkat || ""} ${s.nama_rombel}`;
  else if (s.diterima_dikelas)
    rombel = `${LANG.js_candidate} ${s.diterima_dikelas}`;
...
function loadKelurahanOptions(kecamatanName, selectedKelurahan = "") {
  ...
  fetch(
    baseUrl +
      "/admin/siswa/getKelurahan?kecamatan=" +
      encodeURIComponent(kecamatanName),
  )
    .then((r) => (r.ok ? r.json() : Promise.reject(r.status)))
    .then((data) => {
  ...
  if (file) {
    if (file.size > 5 * 1024 * 1024) {
      if (typeof Swal !== "undefined")
        Swal.fire("Error", "Maksimal ukuran foto adalah 5MB.", "error");
      else alert("Maksimal ukuran foto adalah 5MB.");
      event.target.value = "";
      return;
    }
    const objectUrl = URL.createObjectURL(file);
    container.innerHTML = `<img src="${objectUrl}" class="w-full h-full object-cover rounded-xl shadow-sm">`;

    container.querySelector("img").onload = function () {
      URL.revokeObjectURL(this.src); // Langsung buang dari RAM agar HP tidak hang!
    };
  } else {
    container.innerHTML = `<svg class="w-8 h-8 text-gray-300 dark:text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" /></svg>`;
  }
};

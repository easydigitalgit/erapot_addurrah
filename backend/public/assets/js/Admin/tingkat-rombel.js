...
const defaultConfig = {
    ...
    // Mengambil Tahun Ajaran dinamis dari View (PHP)
    academic_year: typeof window.DYNAMIC_YEAR !== 'undefined' ? window.DYNAMIC_YEAR : '2024/2025'
};

let config = { ...defaultConfig };
const txtClass = LANG?.class_word || 'Kelas';
const txtActive = LANG?.active || 'Aktif';

const levels = [
    { id: 7, name: `${txtClass} 7`, code: 'VII', rombel_count: window.tingkatStats[7].rombel, student_count: window.tingkatStats[7].siswa, laki: window.tingkatStats[7].laki, perempuan: window.tingkatStats[7].perempuan, status: txtActive },
    { id: 8, name: `${txtClass} 8`, code: 'VIII', rombel_count: window.tingkatStats[8].rombel, student_count: window.tingkatStats[8].siswa, laki: window.tingkatStats[8].laki, perempuan: window.tingkatStats[8].perempuan, status: txtActive },
    { id: 9, name: `${txtClass} 9`, code: 'IX', rombel_count: window.tingkatStats[9].rombel, student_count: window.tingkatStats[9].siswa, laki: window.tingkatStats[9].laki, perempuan: window.tingkatStats[9].perempuan, status: txtActive }
];
...
function updateUI() {
    ...
    // Update tulisan tahun ajaran di pojok kanan atas Header
    ...
    if(document.getElementById('page-title')) document.getElementById('page-title').textContent = LANG?.page_title || 'Tingkat & Rombel';
    if(document.getElementById('page-subtitle')) document.getElementById('page-subtitle').textContent = LANG?.page_subtitle || 'Kelola struktur kelas dan rombongan belajar';
}
...
function populateLevels() {
    ...
    const txtRombelCount = LANG?.js_class_count || 'kelas';
    const txtStudentCount = LANG?.js_student_count || 'siswa';
    const txtViewStats = LANG?.js_view_level_stats || 'Lihat Statistik Tingkat';
    const txtRombel = LANG?.study_group || 'Rombel'; 
    const txtTotalSiswa = LANG?.total_student || 'Total Siswa';
    ...
    container.innerHTML = levels.map(level => {
        ...
        <div class="level-card p-4 border-2 rounded-xl hover:shadow-md transition-all cursor-pointer ${bgClass}" onclick="selectLevel(${level.id})" style="${selectedLevel === level.id ? 'border-color: ' + window.themePrimary : ''}">
            ...
            <button onclick="event.stopPropagation(); showDetailTingkat(${level.id})" class="w-full px-3 py-2 text-xs bg-gray-50 dark:bg-slate-800 text-gray-700 dark:text-slate-300 font-bold rounded-lg hover:bg-gray-100 dark:hover:bg-slate-700 transition-colors border border-gray-200 dark:border-slate-600 outline-none flex items-center justify-center gap-2">
...
function selectLevel(levelId) {
    const txtAllLevels = LANG?.all_levels || 'Semua Tingkat';
    if (selectedLevel === levelId) {
        ...
        let tingkatFilter = '';
        if(levelId === 7) tingkatFilter = 'VII';
        if(levelId === 8) tingkatFilter = 'VIII';
        if(levelId === 9) tingkatFilter = 'IX';
        filteredRombels = rombelsData.filter(r => r.tingkat === tingkatFilter);
    }
    
    populateLevels();
    filterTableDOM(selectedLevel);
}

function filterTableDOM(levelId) {
    let tingkatFilter = '';
    if(levelId === 7) tingkatFilter = `${txtClass} VII`;
    if(levelId === 8) tingkatFilter = `${txtClass} VIII`;
    if(levelId === 9) tingkatFilter = `${txtClass} IX`;

    const rows = document.querySelectorAll('#rombelTableBody tr');
    rows.forEach(row => {
        if(row.querySelector('td[colspan]')) return;
        const cellTingkat = row.cells[1].innerText.trim();
        if(!levelId || cellTingkat === tingkatFilter) {
            row.style.display = ''; 
        } else {
...
if(document.getElementById('searchRombel')) {
    document.getElementById('searchRombel').addEventListener('input', (e) => {
...
window.showDetailTingkat = function(levelId) {
    const level = levels.find(l => l.id === levelId);
    if (!level) return;

    document.getElementById('detailTingkatTitle').textContent = level.name;
    document.getElementById('detailTingkatRombel').textContent = level.rombel_count;
    document.getElementById('detailTingkatSiswa').textContent = level.student_count;
    document.getElementById('detailTingkatLaki').textContent = level.laki;
    document.getElementById('detailTingkatPerempuan').textContent = level.perempuan;
...
// ==========================================
function showDetail(rombelId) {
    currentRombelId = rombelId;
    document.getElementById('drawerRombelIcon').textContent = '...';
    document.getElementById('drawerRombelName').textContent = LANG?.js_loading || 'Memuat...';
    ...
    const txtHomeroom = LANG?.js_homeroom || 'Wali Kelas:';
    if(teacherNameEl) teacherNameEl.textContent = `${txtHomeroom} ${LANG?.js_loading || 'Memuat...'}`;
    
    const studentList = document.getElementById('drawerStudentList');
    if(studentList) studentList.innerHTML = `<p class="text-center text-sm font-medium text-gray-400 py-6">${LANG?.js_loading_db || 'Memuat data dari database...'}</p>`;
    ...
    .then(data => {
        ...
        document.getElementById('drawerRombelLevel').textContent = `${LANG?.th_level || 'Tingkat'} ` + data.tingkat;
        
        // KUNCI: AMBIL TAHUN AJARAN DINAMIS DARI CONTROLLER, ATAU FALLBACK KE CONFIG
        document.getElementById('drawerRombelYear').textContent = data.nama_tahun_ajaran || config.academic_year || 'Belum Diset';
            ...
            if(data.siswa && data.siswa.length > 0) {
                ...
                data.siswa.forEach(s => {
                    const genderIcon = s.jenis_kelamin === 'L' 
                        ? `<span class="bg-blue-100 text-blue-600 px-2 py-0.5 rounded text-[10px] font-bold">${LANG?.male || 'Laki-laki'}</span>` 
                        : `<span class="bg-pink-100 text-pink-600 px-2 py-0.5 rounded text-[10px] font-bold">${LANG?.female || 'Perempuan'}</span>`;
...
window.showStudentManagement = function(rombelId) {
    ...
    document.getElementById('studentModalLevel').textContent = `${LANG?.class_word || 'Kelas'} ` + rombel.tingkat;
...
window.closeAddRombelModal = function() {
    ...
    if(document.getElementById('modalRombelTitle')) document.getElementById('modalRombelTitle').innerText = LANG?.modal_add_title || "Tambah Rombel Baru";
}
...
window.editRombel = function(id) {
    ...
    .then(data => {
        ...
        document.querySelector('[name="rombel_name"]').value = data.nama_rombel;
        document.querySelector('[name="level"]').value = data.tingkat;
        document.querySelector('[name="homeroom_teacher"]').value = data.wali_kelas_id || '';

        document.getElementById('modalRombelTitle').innerText = LANG?.modal_edit_title || "Edit Data Rombel";
        showAddRombelModal();
    })
...
window.handleRombelSubmit = function(event) {
    ...
    const successTitle = id ? (LANG?.js_success_updated || 'Berhasil Diperbarui!') : (LANG?.js_success_added || 'Berhasil Ditambahkan!');
    ...
    .then(data => {
        if(data.status === 'success') {
            ...
            Swal.fire(LANG?.js_failed || 'Gagal', data.message, 'error');
        }
    })
    .catch(err => Swal.fire('Error', LANG?.js_error_system || 'Terjadi kesalahan sistem', 'error'))
...
window.confirmDeleteRombel = function() {
    ...
    .then(data => {
        closeDeleteModal(); 
        if(data.status === 'success') {
            Swal.fire({ icon: 'success', title: LANG?.js_deleted || 'Terhapus!', text: data.message, showConfirmButton: false, timer: 1500
            }).then(() => location.reload());
        } else {
            Swal.fire(LANG?.js_failed || 'Gagal', data.message, 'error');
        }
    ...
    .catch(err => {
        closeDeleteModal();
        Swal.fire('Error', LANG?.js_error_server || 'Gagal menghubungi server', 'error');
    });
}
...
window.handleImport = async function(event) {
    ...
    btn.innerHTML = LANG?.js_analyzing || 'Menganalisis & Upload...';
    btn.disabled = true;
    ...
    try {
        ...
        if (data.status === 'success') {
            Swal.fire({ icon: 'success', title: LANG?.js_success || 'Berhasil', text: data.message, timer: 2000, showConfirmButton: false
            }).then(() => { closeImportModal(); window.location.reload(); });
        } else {
            Swal.fire(LANG?.js_failed || 'Gagal', data.message, 'error');
        }
    } catch (error) {
        Swal.fire('Error', LANG?.js_error_connection || 'Koneksi ke server terputus.', 'error');
    } finally {

document.addEventListener('DOMContentLoaded', function() {
    ...
    const baseUrl = baseUrlElement ? baseUrlElement.getAttribute('data-url') : '/';
    ...
    if (inputRole) {
        inputRole.addEventListener('change', function() {
            ...
            const roleKey = roleName ? roleName.toLowerCase().replace(/\s+/g, '') : '';

            if (roleKey === 'guru' || roleKey === 'tendik' || roleKey === 'walikelas' || roleKey === 'superadmin') {
                if(usernameHint && inputId.value === '') usernameHint.classList.remove('hidden');
                ...
                dataGuru.forEach(guru => {
                    ...
                    opt.value = guru.nuptk ? guru.nuptk : guru.nama.replace(/\s+/g, '').toLowerCase(); 
                ...
                dataOrangTua.forEach(ortu => {
                    const opt = document.createElement('option');
                    opt.value = ortu.nama.replace(/\s+/g, '').toLowerCase() + Math.floor(Math.random() * 100); 
                ...
                dataTahfidz.forEach(tahfidz => {
                    const opt = document.createElement('option');
                    opt.value = tahfidz.nuptk ? tahfidz.nuptk : tahfidz.nama.replace(/\s+/g, '').toLowerCase(); 
    ...
    if (inputUsername) {
        inputUsername.addEventListener('input', function() {
            ...
            const roleName = inputRole.options[inputRole.selectedIndex].getAttribute('data-name');
            const roleKey = roleName ? roleName.toLowerCase().replace(/\s+/g, '') : '';
            
            let currentSource = [];
            if (roleKey === 'guru' || roleKey === 'tendik' || roleKey === 'walikelas') currentSource = dataGuru;
            else if (roleKey === 'siswa') currentSource = dataSiswa;
            else if (roleKey === 'orangtua' || roleKey === 'wali') currentSource = dataOrangTua;
            else if (roleKey === 'gurutahfidzh' || roleKey === 'gurutahfidz') currentSource = dataTahfidz;

            selectedData = currentSource.find(item => {
                const val = item.nuptk || item.nis || item.nama.replace(/\s+/g, '').toLowerCase();
                return val == typedValue;
            });

            if (selectedData) {
                if(linkedIdInput) linkedIdInput.value = selectedData.id;
                if(inputEmail) inputEmail.value = selectedData.email || ''; 
    ...
    if(formUser) {
        formUser.addEventListener('submit', async function(e) {
            ...
            if(inputPassword.value !== '' && inputPassword.value !== inputConfirm.value) {
            ...
            if (!isEdit) {
                ...
                const requiredRoles = ['guru', 'tendik', 'walikelas', 'siswa', 'orangtua', 'wali', 'gurutahfidzh', 'gurutahfidz'];
            ...
            const targetUrl = isEdit ? baseUrl + 'admin/users/update' : baseUrl + 'admin/users/store';
    ...
    function applyFilters() {
        ...
        if (roleValue) params.set('role', roleValue); else params.delete('role');
    ...
    if (filterRoleSelect) filterRoleSelect.addEventListener('change', applyFilters);
    ...
    window.showAddUserModal = function() {
        ...
        if(inputPassword) { inputPassword.type = "password"; inputPassword.required = true; }
        if(inputConfirm) { inputConfirm.type = "password"; inputConfirm.required = true; }
    ...
    window.openEditModal = function(userData) {
        ...
        if(inputFullname) inputFullname.value = userData.full_name || userData.nama_lengkap || ''; 
        ...
        if(inputRole) {
            inputRole.value = (userData.role_id && userData.role_id != 0) ? userData.role_id : "";
        }

        if(inputPassword) { inputPassword.value = ''; inputPassword.required = false; }
        if(inputConfirm) { inputConfirm.value = ''; inputConfirm.required = false; }
        ...
        let roleIdsString = userData.all_roles_ids ? String(userData.all_roles_ids) : String(userData.role_id);
            ...
            bars.forEach((bar, idx) => {
                if(bar) {
                    if(idx < score) {
                        ...
                        bar.className = 'h-full w-1/4 bg-slate-200 rounded-full transition-all';
                    }
                }
            });
            
            if(text) {
                text.innerText = score < 2 ? LANG.js_pass_weak : score < 3 ? LANG.js_pass_medium : LANG.js_pass_strong;
    ...
    window.openDetailModal = function(userData) {
        if(detailDrawer && drawerOverlay) {
            ...
            setTimeout(() => { detailDrawer.classList.remove('translate-x-full'); }, 10);
        }
        document.body.style.overflow = 'hidden';

        const v = (val) => val ? val : '-';
        
        if(document.getElementById('drawerName')) document.getElementById('drawerName').innerText = v(userData.username);
        if(document.getElementById('drawerEmail')) document.getElementById('drawerEmail').innerText = v(userData.email);
        
        const displayName = userData.full_name || userData.username || 'US';
        ...
        let avatarUrl = '';
        if (userData.foto_profil && userData.foto_profil !== 'null' && String(userData.foto_profil).trim() !== '') {
            const cleanBaseUrl = baseUrl.replace(/\/$/, ''); 
            const isSiswa = (userData.role_id == 4 || String(userData.role_name).toLowerCase() === 'siswa');
            const folderTarget = isSiswa ? 'siswa' : 'avatars';
            
            avatarUrl = `${cleanBaseUrl}/assets/uploads/${folderTarget}/${userData.foto_profil}`;
        } else {
        ...
        if(document.getElementById('info-username')) document.getElementById('info-username').innerText = v(userData.username);
        
        if(document.getElementById('info-joined') && userData.created_at) {
            const d = new Date(userData.created_at);
            document.getElementById('info-joined').innerText = new Intl.DateTimeFormat(document.documentElement.lang || 'id-ID', { dateStyle: 'medium' }).format(d);
        }

        if(document.getElementById('status-id-input')) document.getElementById('status-id-input').value = userData.id;
        if(document.getElementById('detail-id-input')) document.getElementById('detail-id-input').value = userData.id;

        const toggle = document.getElementById('toggle-status-akun');
        const label = document.getElementById('status-text-label');
        if(toggle) {
            toggle.checked = isActive;
            if(label) label.innerText = isActive ? LANG.status_active : LANG.status_inactive;
            
            const newToggle = toggle.cloneNode(true);
            toggle.parentNode.replaceChild(newToggle, toggle);
            
            newToggle.addEventListener('change', function() {
                const url = this.checked ? baseUrl + 'admin/users/activate' : baseUrl + 'admin/users/deactivate';
                const fd = new FormData();
                fd.append('id', userData.id);
                fetch(url, { method: 'POST', body: fd }).then(res => { if(res.ok) window.location.reload(); });
            });
        }
    ...
    window.executeDeleteAction = async function() {
        ...
        const btn = document.querySelector('#delete-modal button[onclick="executeDeleteAction()"]');
    ...
    window.confirmBulkDelete = function() {
        ...
        if (confirm(LANG.js_delete_selected.replace('{count}', checkedBoxes.length))) {
...
window.updateInlineRoles = function(userId) {
    ...
    const baseUrlRaw = document.getElementById('base-url').dataset.url.replace(/\/$/, '');

...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (personalInfoForm) {
        personalInfoForm.addEventListener('submit', async function(e) {
            ...
            try {
                const response = await fetch(BASE_URL + 'admin/akun-saya/update-profile', {
    ...
    if (pwdForm) {
        pwdForm.addEventListener('submit', async function(e) {
            ...
            try {
                const response = await fetch(BASE_URL + 'admin/akun-saya/update-password', {
                    method: 'POST', body: formData, headers: { 'X-Requested-With': 'XMLHttpRequest' }
                });
                const result = await response.json();
...
async function savePreferences() {
    ...
    try {
        const response = await fetch(BASE_URL + 'admin/akun-saya/update-preferences', {
            method: 'POST', body: formData, headers: { 'X-Requested-With': 'XMLHttpRequest' }
        });
        const result = await response.json();

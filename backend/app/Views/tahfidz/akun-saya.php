...
<style>
  ...
  .avatar-upload-overlay { transition: all 0.3s ease; }
  .radio-custom { width: 1.25rem; height: 1.25rem; cursor: pointer; }
</style>
<?= $this->endSection() ?>
    ...
    <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 mb-6">
        <div>
            <h1 class="text-2xl md:text-3xl font-bold text-gray-900 dark:text-white mb-2 flex items-center gap-3">
                <svg class="w-8 h-8 text-[var(--warna-primary)]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" /></svg> 
...
<div class="mb-6 rounded-2xl shadow-lg overflow-hidden border-0">
    <div class="bg-gradient-to-r from-[var(--warna-primary)] to-[var(--warna-primary)]/70 p-6 md:p-8 text-white relative">
        ...
        <div class="flex flex-col md:flex-row gap-6 items-center md:items-start relative z-10">
            <?php 
                $namaLengkap = session()->get('nama_lengkap') ?? $user['nama_lengkap'] ?? 'Guru Tahfidz';
...
<div class="card mb-6 bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700">
    <h3 class="text-xl font-bold text-gray-900 dark:text-white mb-6 flex items-center gap-2">
        <svg class="w-6 h-6 text-[var(--warna-primary)]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg> 
...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
    <div class="card bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700">
        <h3 class="text-xl font-bold text-gray-900 dark:text-white mb-6 flex items-center gap-2">
            <svg class="w-6 h-6 text-[var(--warna-primary)]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" /></svg> 
        ...
        <div class="p-4 bg-[var(--warna-secondary)] border border-[var(--warna-primary)] rounded-xl">
            <div class="flex items-start gap-2">
                <svg class="w-5 h-5 text-[var(--warna-primary)] flex-shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                <div>
...
<div class="card mb-6 bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700">
    <h3 class="text-xl font-bold text-gray-900 dark:text-white mb-6 flex items-center gap-2">
        <svg class="w-6 h-6 text-[var(--warna-primary)]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg> 
        ...
        <table class="w-full text-left">
            ...
            <tbody class="divide-y divide-gray-100 dark:divide-slate-700">
                <?php if(empty($login_logs)): ?>
                    <tr><td colspan="4" class="text-center py-6 text-gray-500 dark:text-slate-400"><?= lang('Tahfidz/AkunSaya.empty_login') ?></td></tr>
                <?php else: ?>
    ...
    <div class="bg-white dark:bg-slate-800 rounded-3xl w-full max-w-md p-6 transform scale-95 transition-transform duration-300 shadow-2xl border border-gray-100 dark:border-slate-700">
        <div class="text-center mb-6">
            <div class="w-16 h-16 rounded-full bg-[var(--warna-primary)]/10 flex items-center justify-center mx-auto mb-4 border-2 border-[var(--warna-primary)]/20">
                <svg class="w-8 h-8 text-[var(--warna-primary)]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 7a2 2 0 012 2m4 0a6 6 0 01-7.743 5.743L11 17H9v2H7v2H4a1 1 0 01-1-1v-2.586a1 1 0 01.293-.707l5.964-5.964A6 6 0 1121 9z" /></svg>
            </div>
        ...
        <form id="changePasswordForm">
            <div class="space-y-4 mb-6">
                <div>
                    <label class="block text-sm font-semibold text-gray-700 dark:text-slate-300 mb-1"><?= lang('Tahfidz/AkunSaya.old_pass') ?></label> 
                    <input type="password" id="oldPassword" class="w-full px-4 py-2.5 border border-gray-200 dark:border-slate-600 bg-gray-50 dark:bg-slate-700 text-gray-900 dark:text-white rounded-xl focus:ring-2 focus:ring-[var(--warna-primary)] outline-none transition-all shadow-sm" required>
                </div>
                <div>
                    <label class="block text-sm font-semibold text-gray-700 dark:text-slate-300 mb-1"><?= lang('Tahfidz/AkunSaya.new_pass') ?></label> 
                    <input type="password" id="newPassword" class="w-full px-4 py-2.5 border border-gray-200 dark:border-slate-600 bg-gray-50 dark:bg-slate-700 text-gray-900 dark:text-white rounded-xl focus:ring-2 focus:ring-[var(--warna-primary)] outline-none transition-all shadow-sm" required>
                    <p class="text-xs text-gray-500 dark:text-slate-400 mt-1.5"><?= lang('Tahfidz/AkunSaya.pass_min_char') ?></p>
                </div>
                <div>
                    <label class="block text-sm font-semibold text-gray-700 dark:text-slate-300 mb-1"><?= lang('Tahfidz/AkunSaya.conf_new_pass') ?></label> 
                    <input type="password" id="confirmPassword" class="w-full px-4 py-2.5 border border-gray-200 dark:border-slate-600 bg-gray-50 dark:bg-slate-700 text-gray-900 dark:text-white rounded-xl focus:ring-2 focus:ring-[var(--warna-primary)] outline-none transition-all shadow-sm" required>
                </div>
            </div>

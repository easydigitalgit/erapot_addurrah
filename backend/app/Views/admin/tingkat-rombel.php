...
<div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
    ...
    <div class="stat-card bg-white dark:bg-slate-800 rounded-2xl p-5 shadow-sm border border-gray-100 dark:border-slate-700 transition-colors">
        ...
        <h3 class="text-2xl font-bold text-gray-800 dark:text-white"><?= $total_rombel ?></h3>
    </div>
    <div class="stat-card bg-white dark:bg-slate-800 rounded-2xl p-5 shadow-sm border border-gray-100 dark:border-slate-700 transition-colors">
        <div class="flex items-center gap-3 mb-2">
            <div class="w-10 h-10 rounded-xl bg-purple-100 dark:bg-purple-900/30 flex items-center justify-center">
                <svg class="w-5 h-5 text-purple-600 dark:text-purple-400" fill="none" stroke="currentColor" viewbox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z" /></svg>
            </div>
        </div>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center gap-3">
                                            ...
                                            <div>
                                                ...
                                                <p class="text-[11px] text-gray-500 dark:text-slate-400 font-medium"><?= lang('Admin/TingkatRombel.academic_year') ?>: <?= esc($row['nama_tahun_ajaran']) ?></p>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        ...
                                        <div class="flex items-center gap-2">
                                            ...
                                            <?php else : ?>
                                                <span class="text-xs text-amber-600 dark:text-amber-500 font-bold flex items-center gap-1 bg-amber-50 dark:bg-amber-900/30 px-2 py-0.5 rounded border border-amber-200 dark:border-amber-800/50">
                                                    <?= lang('Admin/TingkatRombel.no_homeroom_yet') ?>
                                                </span>
                                            <?php endif; ?>
                                        </div>
                                    </td>
...
<div id="detailDrawer" class="drawer fixed inset-y-0 right-0 w-full md:w-[450px] bg-white dark:bg-slate-800 shadow-2xl transform translate-x-full transition-transform duration-300 ease-in-out flex flex-col border-l border-gray-200 dark:border-slate-700" style="z-index: 999998 !important;">
    ...
    <div class="p-5 border-t border-gray-100 dark:border-slate-700 bg-gray-50 dark:bg-slate-800/80 z-10 flex gap-3 transition-colors">
        <button onclick="editRombel(document.getElementById('detailDrawer').dataset.rombelId)" class="flex-1 px-4 py-2.5 text-white font-bold rounded-xl transition-all transform hover:-translate-y-0.5 shadow-md flex items-center justify-center gap-2 outline-none" style="background-color: <?= $color['warna_primary'] ?? '#10b981' ?>;">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewbox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg> <?= lang('Admin/TingkatRombel.btn_edit_rombel') ?>
        </button>
    ...
    <div class="absolute inset-0 flex items-center justify-center p-4 pointer-events-none md:left-64">
        <div class="relative w-full bg-white dark:bg-slate-800 rounded-2xl shadow-2xl flex flex-col max-h-[90vh] pointer-events-auto border border-transparent dark:border-slate-700 transition-colors modal-content" style="max-width: 500px;">
            ...
            <form id="formTambahRombel" class="p-6 space-y-4" onsubmit="handleRombelSubmit(event)">
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-2"><?= lang('Admin/TingkatRombel.academic_year') ?> (Aktif)</label>
                    <input type="text" name="academic_year" value="<?= $strTaAktif ?>" readonly class="w-full px-4 py-2.5 bg-gray-100 dark:bg-slate-900/50 border border-gray-200 dark:border-slate-700 rounded-xl text-sm font-semibold text-gray-500 dark:text-slate-400 cursor-not-allowed outline-none shadow-sm">
                </div>
            ...
            <div class="p-6 md:p-8 flex-1 overflow-y-auto custom-scrollbar">
                ...
                <div id="currentStudentsTab" class="student-tab">
                    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-4 gap-3">
                        ...
                        <div class="relative w-full sm:w-auto">
                            ...
                            <svg class="w-5 h-5 text-gray-400 dark:!text-slate-500 absolute left-3 top-1/2 -translate-y-1/2 pointer-events-none" fill="none" stroke="currentColor" viewbox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>
                        </div>
                    </div>
                ...
                <div id="addStudentsTab" class="student-tab hidden">
                    <div class="mb-4 relative">
                        ...
                        <svg class="w-5 h-5 text-gray-400 dark:!text-slate-500 absolute left-4 top-9 pointer-events-none" fill="none" stroke="currentColor" viewbox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>
                    </div>
...
<div id="deleteRombelModal" class="fixed inset-0 z-[100] hidden">
    ...
    <div class="absolute inset-0 flex items-center justify-center p-4 pointer-events-none md:left-64">
        <div class="relative w-full bg-white dark:bg-slate-800 rounded-2xl shadow-2xl flex flex-col max-w-md pointer-events-auto border border-transparent dark:border-slate-700 transition-colors">
            <div class="p-8 text-center">
                ...
                <h3 class="text-2xl font-bold text-gray-800 dark:text-white mb-2"><?= lang('Admin/TingkatRombel.modal_delete_title') ?></h3>
                <p class="text-sm font-medium text-gray-500 dark:text-slate-400 mb-8 px-4 leading-relaxed"><?= lang('Admin/TingkatRombel.modal_delete_desc') ?></p>
                <input type="hidden" id="deleteRombelId">
                <div class="flex gap-3">
                    <button onclick="closeDeleteModal()" class="flex-1 px-5 py-3 bg-white dark:bg-slate-700 border border-gray-300 dark:border-slate-600 text-gray-700 dark:text-slate-300 font-bold rounded-xl hover:bg-gray-50 dark:hover:bg-slate-600 transition-colors outline-none shadow-sm"><?= lang('Admin/TingkatRombel.btn_cancel') ?></button>
                    <button onclick="confirmDeleteRombel()" class="flex-1 px-5 py-3 bg-red-600 text-white font-bold rounded-xl hover:bg-red-700 shadow-lg shadow-red-600/30 transition-colors outline-none"><?= lang('Admin/TingkatRombel.btn_yes_delete') ?></button>
                </div>
            </div>
        </div>
    </div>
            ...
            <form id="importForm" action="<?= base_url('admin/rombel/import') ?>" method="POST" onsubmit="handleImport(event)" enctype="multipart/form-data" class="p-6 space-y-5">
                ...
                <div class="flex gap-3 pt-4 border-t border-gray-100 dark:border-slate-700">
                    ...
                    <button type="submit" class="flex-1 px-5 py-3 text-white font-bold rounded-xl shadow-lg transition-transform flex items-center justify-center gap-2 outline-none transform hover:-translate-y-0.5" style="background-color: <?= $color['warna_primary'] ?? '#10b981' ?>; box-shadow: 0 10px 15px -3px <?= $color['warna_primary'] ?? '#10b981' ?>40;">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewbox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12" /></svg> <?= lang('Admin/TingkatRombel.btn_upload_import') ?>
                    </button>
                </div>
            </form>
        </div>
...
<script>
    ...
    let rawLang = <?= json_encode(lang('Admin/TingkatRombel')) ?>;
    const LANG = typeof rawLang === 'object' ? rawLang : {};
    
    window.rombels = <?= json_encode($rombel_list ?? []) ?>;
    window.tingkatStats = {
        7: <?= json_encode($stat_7 ?? ['rombel'=>0, 'siswa'=>0, 'laki'=>0, 'perempuan'=>0]) ?>,
        8: <?= json_encode($stat_8 ?? ['rombel'=>0, 'siswa'=>0, 'laki'=>0, 'perempuan'=>0]) ?>,
        9: <?= json_encode($stat_9 ?? ['rombel'=>0, 'siswa'=>0, 'laki'=>0, 'perempuan'=>0]) ?>
    };
    
    window.themePrimary = "<?= $color['warna_primary'] ?? '#10b981' ?>";
    
    // KUNCI PERBAIKAN: Menarik Tahun Ajaran Aktif untuk JS Config

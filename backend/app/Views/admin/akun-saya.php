...
<div class="mb-6">
    ...
    <div class="bg-[<?= $color['warna_secondary'] ?>] dark:bg-slate-900/80 border border-[<?= $color['warna_primary'] ?>] p-4 rounded-xl mb-6">
        <div class="flex items-start gap-3">
            <div class="w-10 h-10 rounded-lg bg-[<?= $color['warna_secondary'] ?>] dark:bg-slate-800 flex items-center justify-center flex-shrink-0">
            ...
            <div class="flex-1">
                ...
                <ul class="text-sm text-[<?= $color['warna_primary'] ?>] space-y-1">
                    <li class="flex items-center gap-2">
                        <svg class="w-4 h-4 text-[<?= $color['warna_primary'] ?>]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
<div class="mb-6 rounded-2xl shadow-lg overflow-hidden border-0">
    <div class="bg-gradient-to-r from-[<?= $color['warna_primary'] ?>] to-[<?= $color['warna_primary'] ?>]/70 p-6 md:p-8 text-white relative">
        ...
        <div class="flex flex-col md:flex-row gap-6 items-center md:items-start relative z-10">
            ...
            // 4. Logika Penentuan URL Foto Profil
            if (!empty($fotoProfil) && file_exists(FCPATH . 'assets/uploads/avatars/' . $fotoProfil)) {
                // TAMBAHKAN PENCHANCUR CACHE DI SINI:
                $avatarUrl = base_url('assets/uploads/avatars/' . $fotoProfil) . '?v=' . time();
            } else {
                $avatarUrl = "https://ui-avatars.com/api/?name=" . urlencode($inisial) . "&background=1F7A4D&color=fff&size=160&bold=true&rounded=true";
            }
            ?>
...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
    <div class="card bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700">
        ...
        <div class="space-y-4">
            ...
            <div class="p-4 bg-[<?= $color['warna_secondary'] ?>] dark:bg-slate-800 border border-[<?= $color['warna_primary'] ?>] rounded-lg">
        ...
        <table class="activity-table w-full text-left ">
            <thead class="border-b dark:border-slate-700 ">
                <tr class="dark:bg-slate-600">
                    <th class="py-3 dark:bg-slate-600 px-4 text-sm font-semibold text-gray-600 dark:text-slate-300 dark:bg-slate-700"><?= lang('Admin/Akun.th_time') ?></th>
                    <th class="py-3 dark:bg-slate-600 px-4 text-sm font-semibold text-gray-600 dark:text-slate-300 "><?= lang('Admin/Akun.th_device') ?></th>
                    <th class="py-3 dark:bg-slate-600 px-4 text-sm font-semibold text-gray-600 dark:text-slate-300"><?= lang('Admin/Akun.th_ip') ?></th>
                    <th class="py-3 dark:bg-slate-600 px-4 text-sm font-semibold text-gray-600 dark:text-slate-300"><?= lang('Admin/Akun.th_status') ?></th>
                </tr>
            </thead>
            <tbody class="divide-y divide-gray-100 dark:divide-slate-700">
                <?php if (empty($login_logs)): ?>
                    <tr>
                        <td colspan="4" class="text-center py-6 text-gray-500 dark:text-slate-400"><?= lang('Admin/Akun.empty_login') ?></td>
                    </tr>
                <?php else: ?>
        ...
        <div class="space-y-4">
            <div class="info-panel">
                <div class="flex items-center gap-3 mb-3">
                    <div class="w-10 h-10 rounded-lg bg-[<?= $color['warna_secondary'] ?>] dark:bg-slate-900/70 flex items-center justify-center">
                        <svg class="w-5 h-5 text-[<?= $color['warna_primary'] ?>]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" />
                        </svg>
                    </div>

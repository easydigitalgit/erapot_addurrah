...
<?= $this->section('title') ?>
  Backup Sistem - Rapor Digital SMPIT Ad Durrah
<?= $this->endSection() ?>
    ...
    <div class="mb-6">
      ...
      <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 mb-6">
       <div>
        <h1 class="text-2xl md:text-3xl font-bold text-gray-900 dark:text-white mb-2 flex items-center gap-3 transition-colors">
         ...
         </svg> Backup Data Sistem</h1>
        <p class="text-sm md:text-base text-gray-600 dark:text-slate-400 transition-colors">Amankan seluruh data akademik dan sistem secara berkala.</p>
       </div>
       <div class="flex flex-wrap items-center gap-2">
         ...
         </svg><span>Backup Sekarang</span> </button>
       </div>
      </div>
      ...
      <div class="bg-gradient-to-r from-[<?= $color['warna_secondary'] ?>] to-white dark:from-slate-800 dark:to-slate-800/80 border border-[<?= $color['warna_primary'] ?>]/50 p-5 rounded-2xl mb-8 shadow-sm transition-colors">
       <div class="flex items-start gap-4">
        ...
        <div class="flex-1 min-w-0">
         <h4 class="font-bold text-[<?= $color['warna_primary'] ?>] text-lg mb-2 flex items-center gap-2">Proteksi Data Terintegrasi <span class="inline-flex items-center gap-1 bg-[<?= $color['warna_primary'] ?>]/10 text-[<?= $color['warna_primary'] ?>] border border-[<?= $color['warna_primary'] ?>]/20 px-2 py-0.5 rounded-full text-[10px] uppercase font-black tracking-widest shadow-sm">
         ...
         <ul class="text-sm font-medium text-gray-700 dark:text-slate-300 space-y-1.5 transition-colors">
          <li class="flex items-center gap-2">
           ...
           </svg> Setiap backup <strong>terenkripsi</strong> dan tersimpan dengan aman</li>
          <li class="flex items-center gap-2">
           <svg class="w-4 h-4 text-[<?= $color['warna_primary'] ?>] flex-shrink-0" fill="none" stroke="currentColor" viewbox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
           </svg> Semua aktivitas backup dan restore <strong>tercatat dalam audit log</strong></li>
          <li class="flex items-center gap-2">
           <svg class="w-4 h-4 text-[<?= $color['warna_primary'] ?>] flex-shrink-0" fill="none" stroke="currentColor" viewbox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
           </svg> Restore memerlukan <strong>konfirmasi ganda</strong> untuk mencegah kesalahan</li>
         </ul>
        </div>
       </div>
      </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
       ...
       <div class="stat-card bg-white dark:bg-slate-800 border border-gray-100 dark:border-slate-700 rounded-3xl p-5 shadow-sm transition-colors hover:shadow-md hover:border-blue-500 group">
        ...
        <h3 class="text-xs font-black text-gray-500 dark:text-slate-400 uppercase tracking-widest mb-1 transition-colors">Total File</h3>
        <p class="text-3xl font-black text-gray-900 dark:text-white mb-1 transition-colors"><?= $total_files ?> File</p>
        <p class="text-xs font-medium text-gray-500 dark:text-slate-400 mt-1">Tersimpan di direktori server</p>
       </div>
       ...
       <div class="stat-card bg-white dark:bg-slate-800 border border-gray-100 dark:border-slate-700 rounded-3xl p-5 shadow-sm transition-colors hover:shadow-md hover:border-purple-500 group">
        ...
        <h3 class="text-xs font-black text-gray-500 dark:text-slate-400 uppercase tracking-widest mb-1 transition-colors">Total Ukuran</h3>
        <p class="text-3xl font-black text-gray-900 dark:text-white mb-1 transition-colors"><?= $total_size ?></p>
        <p class="text-xs font-medium text-gray-500 dark:text-slate-400 mt-1">Akumulasi seluruh file .sql</p>
       </div>
       ...
       <div class="stat-card bg-white dark:bg-slate-800 border border-gray-100 dark:border-slate-700 rounded-3xl p-5 shadow-sm transition-colors hover:shadow-md hover:border-emerald-500 group">
        ...
        <h3 class="text-xs font-black text-gray-500 dark:text-slate-400 uppercase tracking-widest mb-1 transition-colors">Status Sistem</h3>
        <p class="text-3xl font-black text-emerald-600 dark:text-emerald-400 mb-1 flex items-center gap-2">Normal <span class="w-2.5 h-2.5 bg-emerald-500 rounded-full animate-pulse shadow-[0_0_8px_#10b981]"></span></p>
        <p class="text-xs font-medium text-gray-500 dark:text-slate-400 mt-1">Siap untuk memproses backup</p>
       </div>
       ...
       <div class="stat-card bg-white dark:bg-slate-800 border border-gray-100 dark:border-slate-700 rounded-3xl p-5 shadow-sm transition-colors hover:shadow-md hover:border-amber-500 group">
        ...
        <h3 class="text-xs font-black text-gray-500 dark:text-slate-400 uppercase tracking-widest mb-1 transition-colors">Storage Warning</h3>
        <p class="text-3xl font-black text-gray-900 dark:text-white mb-2 transition-colors">Aman</p>
        ...
        <p class="text-[10px] font-bold text-gray-500 dark:text-slate-400 mt-2 uppercase tracking-wider">Cukup untuk jangka panjang</p>
       </div>
      </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
        ...
        <h3 class="text-xl font-bold text-gray-900 dark:text-white mb-5 flex items-center gap-2">
         ...
         </svg> Pilih Data untuk Backup</h3>
        ...
        <div class="space-y-3" id="backupCategoryOptions">
            <label class="flex items-start gap-4 p-4 rounded-xl border border-gray-100 dark:border-slate-700 bg-gray-50 dark:bg-slate-700/50 hover:border-[<?= $color['warna_primary'] ?>] dark:hover:border-[<?= $color['warna_primary'] ?>] cursor-pointer transition-colors group">
                ...
                <div class="flex-1">
                    <p class="font-bold text-gray-900 dark:text-white text-sm group-hover:text-[<?= $color['warna_primary'] ?>] transition-colors">Data Siswa &amp; Orang Tua</p>
                    <p class="text-[11px] font-medium text-gray-500 dark:text-slate-400 mt-1"><?= number_format($stats['siswa_ortu']['rows'] ?? 0) ?> data • ~<?= ($stats['siswa_ortu']['size_mb'] ?? 0) > 0 ? ($stats['siswa_ortu']['size_mb'] ?? 0) . ' MB' : ($stats['siswa_ortu']['size_kb'] ?? 0) . ' KB' ?></p>
                </div>
            </label> 
            ...
            <label class="flex items-start gap-4 p-4 rounded-xl border border-gray-100 dark:border-slate-700 bg-gray-50 dark:bg-slate-700/50 hover:border-[<?= $color['warna_primary'] ?>] dark:hover:border-[<?= $color['warna_primary'] ?>] cursor-pointer transition-colors group"> 
                ...
                <div class="flex-1">
                    <p class="font-bold text-gray-900 dark:text-white text-sm group-hover:text-[<?= $color['warna_primary'] ?>] transition-colors">Nilai &amp; Rapor Akademik</p>
                    <p class="text-[11px] font-medium text-gray-500 dark:text-slate-400 mt-1"><?= number_format($stats['nilai_rapor']['rows'] ?? 0) ?> baris • ~<?= ($stats['nilai_rapor']['size_mb'] ?? 0) > 0 ? ($stats['nilai_rapor']['size_mb'] ?? 0) . ' MB' : ($stats['nilai_rapor']['size_kb'] ?? 0) . ' KB' ?></p>
                </div>
            </label> 
            ...
            <label class="flex items-start gap-4 p-4 rounded-xl border border-gray-100 dark:border-slate-700 bg-gray-50 dark:bg-slate-700/50 hover:border-[<?= $color['warna_primary'] ?>] dark:hover:border-[<?= $color['warna_primary'] ?>] cursor-pointer transition-colors group"> 
                ...
                <div class="flex-1">
                    <p class="font-bold text-gray-900 dark:text-white text-sm group-hover:text-[<?= $color['warna_primary'] ?>] transition-colors">Master Data Akademik</p>
                    <p class="text-[11px] font-medium text-gray-500 dark:text-slate-400 mt-1">Kelas, mapel, guru • ~<?= ($stats['master']['size_mb'] ?? 0) > 0 ? ($stats['master']['size_mb'] ?? 0) . ' MB' : ($stats['master']['size_kb'] ?? 0) . ' KB' ?></p>
                </div>
            </label> 
            ...
            <label class="flex items-start gap-4 p-4 rounded-xl border border-gray-100 dark:border-slate-700 bg-gray-50 dark:bg-slate-700/50 hover:border-[<?= $color['warna_primary'] ?>] dark:hover:border-[<?= $color['warna_primary'] ?>] cursor-pointer transition-colors group"> 
                ...
                <div class="flex-1">
                    <p class="font-bold text-gray-900 dark:text-white text-sm group-hover:text-[<?= $color['warna_primary'] ?>] transition-colors">Konfigurasi Sistem</p>
                    <p class="text-[11px] font-medium text-gray-500 dark:text-slate-400 mt-1">Pengaturan & Hak akses • ~<?= ($stats['konfigurasi']['size_mb'] ?? 0) > 0 ? ($stats['konfigurasi']['size_mb'] ?? 0) . ' MB' : ($stats['konfigurasi']['size_kb'] ?? 0) . ' KB' ?></p>
                </div>
            </label> 
        </div>
        
        <div class="mt-6 pt-5 border-t border-gray-100 dark:border-slate-700 transition-colors">
         <h4 class="text-xs font-black text-gray-500 dark:text-slate-400 uppercase tracking-widest mb-4">Mode Backup</h4>
         <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <label class="flex items-start gap-3 p-4 rounded-xl border-2 border-gray-200 dark:border-slate-600 cursor-pointer transition-colors hover:border-[<?= $color['warna_primary'] ?>] bg-white dark:bg-slate-800 relative"> 
              ...
              <div class="min-w-0">
               <p class="font-bold text-gray-900 dark:text-white text-sm">Full Backup</p>
               <p class="text-[10px] font-medium text-gray-500 dark:text-slate-400 mt-1">Seluruh data database utuh</p>
              </div>
          </label> 
          <label class="flex items-start gap-3 p-4 rounded-xl border-2 border-gray-200 dark:border-slate-600 cursor-pointer transition-colors hover:border-[<?= $color['warna_primary'] ?>] bg-white dark:bg-slate-800 relative"> 
              ...
              <div class="min-w-0">
               <p class="font-bold text-gray-900 dark:text-white text-sm">Partial (Pilih)</p>
               <p class="text-[10px] font-medium text-gray-500 dark:text-slate-400 mt-1">Hanya yang dicentang di atas</p>
              </div>
          </label>
         </div>
        </div>
       ...
       <div class="bg-white dark:bg-slate-800 rounded-3xl border border-gray-100 dark:border-slate-700 shadow-sm p-6 transition-colors relative">
        <form id="formAutoBackup">
            ...
            <div id="scheduleSettings" class="space-y-5" style="opacity: <?= ($settings['auto_backup'] ?? 1) ? '1' : '0.5' ?>; pointer-events: <?= ($settings['auto_backup'] ?? 1) ? 'auto' : 'none' ?>;">
             <div>
                 <label class="block text-sm font-bold text-gray-700 dark:text-slate-300 mb-2">Frekuensi Backup</label> 
                 <select name="frequency" class="w-full px-4 py-3 border border-gray-200 dark:border-slate-600 bg-white dark:bg-slate-700 text-gray-800 dark:text-white rounded-xl focus:ring-2 focus:ring-[<?= $color['warna_primary'] ?>] transition-colors appearance-none cursor-pointer outline-none shadow-sm"> 
                     ...
                     <option value="weekly" <?= ($settings['frequency'] ?? '') == 'weekly' ? 'selected' : '' ?>>Mingguan</option> 
                     <option value="monthly" <?= ($settings['frequency'] ?? '') == 'monthly' ? 'selected' : '' ?>>Bulanan</option> 
                 </select>
             </div>
             <div>
                 <label class="block text-sm font-bold text-gray-700 dark:text-slate-300 mb-2">Waktu Eksekusi (Server)</label> 
              ...
              <p class="text-[11px] font-medium text-gray-500 dark:text-slate-400 mt-1.5">Backup berjalan via Cronjob pada jam ini</p>
             </div>
             <div>
                 <label class="block text-sm font-bold text-gray-700 dark:text-slate-300 mb-2">Retensi Penyimpanan</label> 
                 <select name="retention_days" class="w-full px-4 py-3 border border-gray-200 dark:border-slate-600 bg-white dark:bg-slate-700 text-gray-800 dark:text-white rounded-xl focus:ring-2 focus:ring-[<?= $color['warna_primary'] ?>] transition-colors appearance-none cursor-pointer outline-none shadow-sm"> 
                     <option value="7" <?= ($settings['retention_days'] ?? 0) == 7 ? 'selected' : '' ?>>Simpan selama 7 Hari</option> 
                     <option value="30" <?= ($settings['retention_days'] ?? 30) == 30 ? 'selected' : '' ?>>Simpan selama 30 Hari</option> 
                     <option value="60" <?= ($settings['retention_days'] ?? 0) == 60 ? 'selected' : '' ?>>Simpan selama 60 Hari</option> 
                 </select>
             </div>
             ...
             <div class="pt-5 border-t border-gray-100 dark:border-slate-700 transition-colors">
                 <label class="flex items-center gap-3 cursor-pointer group"> 
                     ...
                     <span class="text-sm font-bold text-gray-700 dark:text-slate-300 transition-colors">Kirim email notifikasi jika berhasil/gagal</span> 
                 </label>
             </div>
             
             <button type="submit" id="btnSaveSetting" class="w-full mt-2 py-3 bg-[<?= $color['warna_secondary'] ?>] text-[<?= $color['warna_primary'] ?>] border border-[<?= $color['warna_primary'] ?>]/50 font-bold rounded-xl hover:bg-[<?= $color['warna_primary'] ?>] hover:text-white transition-colors">Simpan Pengaturan Jadwal</button>
            </div>
        </form>
       </div>
      </div>
       ...
       <div class="p-6 border-b border-gray-100 dark:border-slate-700 flex items-center justify-between transition-colors">
           <div>
               <h3 class="text-lg font-bold text-gray-900 dark:text-white mb-1 flex items-center gap-2">
                ...
                </svg> Riwayat Backup File</h3>
               <p class="text-sm font-medium text-gray-500 dark:text-slate-400 ml-7">Daftar arsip database yang tersimpan di server</p>
           </div>
        ...
        <table class="w-full text-left border-collapse min-w-max">
         <thead class="bg-gray-50 dark:bg-slate-900/50 border-b border-gray-100 dark:border-slate-700 transition-colors">
          <tr class="text-[11px] text-gray-500 dark:text-slate-400 uppercase tracking-widest font-black">
           ...
           <th class="px-6 py-4">Tipe Backup</th>
             ...
             <tr>
               <td colspan="5" class="text-center py-16 text-gray-500 dark:text-slate-500 font-medium">
                   <div class="flex flex-col items-center gap-3">
                       ...
                       <span>Belum ada file backup yang tersimpan di direktori.</span>
                   </div>
               </td>
             </tr>
           <?php else: ?>
             ...
             <tr class="hover:bg-gray-50/50 dark:hover:bg-slate-700/30 transition-colors group">
              ...
              <td class="px-6 py-4">
                  <?php if($isFull): ?>
                    <span class="inline-flex px-2.5 py-1 text-[10px] font-black uppercase tracking-wider bg-blue-100 text-blue-700 border border-blue-200 rounded-full shadow-sm">Full Backup</span>
                  <?php else: ?>
                    <span class="inline-flex px-2.5 py-1 text-[10px] font-black uppercase tracking-wider bg-amber-100 text-amber-700 border border-amber-200 rounded-full shadow-sm">Partial</span>
                  <?php endif; ?>
              </td>
      ...
      <div class="bg-white dark:bg-slate-800 rounded-3xl border border-gray-100 dark:border-slate-700 shadow-sm p-6 mb-6 transition-colors">
       <div class="flex items-start gap-4 mb-5 border-b border-gray-100 dark:border-slate-700 pb-4 transition-colors">
        ...
        <div class="flex-1 min-w-0">
         <h3 class="text-xl font-bold text-gray-900 dark:text-white mb-1">Restore Database dari Eksternal</h3>
         <p class="text-sm font-medium text-gray-600 dark:text-slate-400">Upload file .sql manual dari device Anda</p>
        </div>
       </div>
       <div class="dropzone w-full border-2 border-dashed border-gray-300 dark:border-slate-600 bg-gray-50/50 dark:bg-slate-700/50 hover:bg-gray-100 dark:hover:bg-slate-700 hover:border-[<?= $color['warna_primary'] ?>] rounded-2xl p-8 text-center cursor-pointer transition-all shadow-sm group mb-6" onclick="document.getElementById('fileInput').click()">
        ...
        <p class="text-sm font-bold text-gray-800 dark:text-white mb-1 group-hover:text-[<?= $color['warna_primary'] ?>] transition-colors">Klik di sini untuk upload file backup</p>
        <p class="text-xs font-medium text-gray-500 dark:text-slate-400">Atau drag &amp; drop file ke area ini</p>
        <p class="text-[10px] font-black uppercase tracking-wider text-gray-400 dark:text-slate-500 mt-3 border border-gray-200 dark:border-slate-600 inline-block px-3 py-1 rounded-full">Format: .sql (Max 500 MB)</p>
       </div>
       <div class="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800/50 p-5 rounded-2xl shadow-sm transition-colors">
        <div class="flex items-start gap-4">
         ...
         <div class="flex-1">
          <h4 class="font-black text-red-900 dark:text-red-400 mb-2 uppercase tracking-wider text-[11px]">Peringatan Kritis Restorasi Data</h4>
          <ul class="text-xs font-medium text-red-800 dark:text-red-300 space-y-1.5 leading-relaxed">
           <li class="flex items-start gap-2"><span class="text-red-600 font-bold">•</span> <span>Melakukan restore akan <strong>menghapus dan menimpa seluruh data saat ini</strong> dengan data dari file backup.</span></li>
           <li class="flex items-start gap-2"><span class="text-red-600 font-bold">•</span> <span>Proses restorasi bersifat permanen dan <strong>tidak dapat dibatalkan (Undo)</strong> setelah dimulai.</span></li>
           <li class="flex items-start gap-2"><span class="text-red-600 font-bold">•</span> <span>Sangat disarankan untuk melakukan <strong>Backup Full Manual</strong> terlebih dahulu sebelum melakukan restore ini.</span></li>
          </ul>
         </div>
        </div>
       </div>
  ...
  <div id="backupModal" class="fixed inset-0 z-[99999] hidden flex items-center justify-center p-4 overflow-y-auto overflow-x-hidden">
    ...
    <div class="relative w-full max-w-md bg-white dark:bg-slate-800 rounded-3xl shadow-2xl p-6 md:p-8 transform transition-colors border border-transparent dark:border-slate-700 scale-100 mx-auto">
      <div class="text-center mb-6">
        ...
        <h3 class="text-2xl font-bold text-gray-900 dark:text-white mb-2 transition-colors">Konfirmasi Eksekusi</h3>
        <p class="text-sm font-medium text-gray-600 dark:text-slate-400 mb-6 transition-colors">Sistem akan mengekstrak tabel sesuai pilihan Anda menjadi arsip (.sql).</p>
        <label class="flex items-start gap-3 bg-white dark:bg-slate-800 border-2 border-gray-200 dark:border-slate-600 p-4 rounded-xl cursor-pointer hover:border-[<?= $color['warna_primary'] ?>] dark:hover:border-[<?= $color['warna_primary'] ?>] transition-colors mb-2 group shadow-sm">
          ...
          <span class="text-sm font-bold text-gray-700 dark:text-slate-300 text-left group-hover:text-gray-900 dark:group-hover:text-white transition-colors">Saya memahami proses backup ini dan siap melanjutkan eksekusi</span>
        </label>
      </div>
  ...
  <div id="restoreModal" class="fixed inset-0 z-[99999] hidden flex items-center justify-center p-4 overflow-y-auto overflow-x-hidden">
    ...
    <div class="relative w-full max-w-md bg-white dark:bg-slate-800 rounded-3xl shadow-2xl p-6 md:p-8 transform transition-colors border border-transparent dark:border-slate-700 scale-100 mx-auto">
      <div class="text-center mb-6">
        ...
        <h3 class="text-2xl font-black text-gray-900 dark:text-white mb-2 transition-colors">Restore Data Sistem</h3>
        <p class="text-sm font-medium text-gray-600 dark:text-slate-400 mb-4 transition-colors">Anda akan mengembalikan sistem menggunakan file:</p>
        ...
        <div class="bg-red-50 dark:bg-red-900/20 border-2 border-red-200 dark:border-red-800/50 p-4 rounded-xl mb-6 text-left shadow-sm transition-colors">
          <h4 class="font-black text-red-900 dark:text-red-400 mb-2 text-xs uppercase tracking-wider">PERINGATAN KRITIS:</h4>
          <p class="text-xs font-medium text-red-800 dark:text-red-300 leading-relaxed">Data saat ini akan <strong>DIHAPUS & DIGANTI SECARA PERMANEN</strong> dengan isi dari file backup tersebut. Kesalahan proses tidak bisa dibatalkan.</p>
        </div>
        ...
        <div class="space-y-3 mb-6">
            <label class="flex items-start gap-3 bg-white dark:bg-slate-800 border-2 border-gray-200 dark:border-slate-600 p-3.5 rounded-xl cursor-pointer hover:border-red-500 dark:hover:border-red-500 transition-colors group">
              ...
              <span class="text-sm font-bold text-gray-700 dark:text-slate-300 text-left group-hover:text-gray-900 dark:group-hover:text-white transition-colors">Saya paham data saat ini akan tertimpa hancur</span>
            </label>
            ...
            <label class="flex items-start gap-3 bg-white dark:bg-slate-800 border-2 border-gray-200 dark:border-slate-600 p-3.5 rounded-xl cursor-pointer hover:border-red-500 dark:hover:border-red-500 transition-colors group">
              ...
              <span class="text-sm font-bold text-gray-700 dark:text-slate-300 text-left group-hover:text-gray-900 dark:group-hover:text-white transition-colors">Saya bertanggung jawab atas restorasi server ini</span>
            </label>
        </div>
      </div>

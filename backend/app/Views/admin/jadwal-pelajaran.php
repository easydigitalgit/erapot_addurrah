...
<div class="mb-6">
  <div class="flex items-center gap-2 text-sm text-gray-500 dark:text-slate-400 mb-2 transition-colors">
    ...
    </svg><span class="text-[<?= $color['warna_primary'] ?>] font-medium"><?= lang('Admin/JadwalPelajaran.page_title') ?></span>
  </div>
...
<div class="bg-white dark:bg-slate-800 rounded-3xl shadow-sm border border-gray-100 dark:border-slate-700 p-6 mb-6 transition-colors">
  <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 mb-4">
    <div>
        <label class="block text-xs font-bold text-gray-600 dark:text-slate-400 uppercase tracking-wider mb-2"><?= lang('Admin/JadwalPelajaran.academic_year') ?></label> 
        <select class="w-full px-4 py-2.5 bg-white dark:bg-slate-700 border border-gray-200 dark:border-slate-600 rounded-xl text-sm font-medium text-gray-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-[<?= $color['warna_primary'] ?>] transition-colors appearance-none cursor-pointer outline-none">
            <option>2024/2025</option>
            <option>2023/2024</option>
        </select>
    </div>
    <div>
        <label class="block text-xs font-bold text-gray-600 dark:text-slate-400 uppercase tracking-wider mb-2"><?= lang('Admin/JadwalPelajaran.semester') ?></label> 
        <select class="w-full px-4 py-2.5 bg-white dark:bg-slate-700 border border-gray-200 dark:border-slate-600 rounded-xl text-sm font-medium text-gray-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-[<?= $color['warna_primary'] ?>] transition-colors appearance-none cursor-pointer outline-none">
    ...
    <div>
        <label class="block text-xs font-bold text-gray-600 dark:text-slate-400 uppercase tracking-wider mb-2"><?= lang('Admin/JadwalPelajaran.level') ?></label> 
        <select id="tingkatFilter" onchange="filterSchedule()" class="w-full px-4 py-2.5 bg-white dark:bg-slate-700 border border-gray-200 dark:border-slate-600 rounded-xl text-sm font-medium text-gray-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-[<?= $color['warna_primary'] ?>] transition-colors appearance-none cursor-pointer outline-none">
  ...
  <div class="flex flex-col sm:flex-row sm:items-center justify-between pt-4 border-t border-gray-100 dark:border-slate-700 gap-4 transition-colors">
      <label class="flex items-center gap-3 cursor-pointer group"> 
          <input type="checkbox" id="showLockedToggle" class="w-5 h-5 rounded border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-[<?= $color['warna_primary'] ?>] focus:ring-[<?= $color['warna_primary'] ?>] cursor-pointer focus:ring-offset-0"> 
          <span class="text-sm font-bold text-gray-700 dark:text-slate-300 group-hover:text-gray-900 dark:group-hover:text-white transition-colors"><?= lang('Admin/JadwalPelajaran.show_locked') ?></span> 
      </label>
      <div class="flex items-center gap-3 bg-gray-50 dark:bg-slate-900/50 py-2 px-4 rounded-xl border border-gray-100 dark:border-slate-700 transition-colors">
          <span id="badgeLockStatus" class="px-2.5 py-1 rounded-full text-[10px] font-black uppercase tracking-wider bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-800/50 shadow-sm"><?= lang('Admin/JadwalPelajaran.status_active') ?></span> 
          <span id="textLockStatus" class="text-xs font-bold text-gray-500 dark:text-slate-400"><?= lang('Admin/JadwalPelajaran.status_editable') ?></span>
      </div>
  </div>
</div>
...
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
  <div class="stat-card bg-white dark:bg-slate-800 rounded-3xl p-5 shadow-sm border border-gray-100 dark:border-slate-700 hover:shadow-md hover:border-[<?= $color['warna_primary'] ?>] transition-all group">
    ...
    <p class="text-3xl font-black text-gray-900 dark:text-white mb-1">48 JP</p>
  ...
  <div class="stat-card bg-white dark:bg-slate-800 rounded-3xl p-5 shadow-sm border border-gray-100 dark:border-slate-700 hover:shadow-md hover:border-[<?= $color['warna_primary'] ?>] transition-all group">
    ...
    <p class="text-3xl font-black text-gray-900 dark:text-white mb-1">24 JP</p>
  ...
  <div class="stat-card bg-white dark:bg-slate-800 rounded-3xl p-5 shadow-sm border border-gray-100 dark:border-slate-700 hover:shadow-md hover:border-[<?= $color['warna_primary'] ?>] transition-all group">
    ...
    <p class="text-3xl font-black text-gray-900 dark:text-white mb-1">2 Slot</p>
  ...
  <div class="stat-card bg-white dark:bg-slate-800 rounded-3xl p-5 shadow-sm border border-gray-100 dark:border-slate-700 hover:shadow-md hover:border-[<?= $color['warna_primary'] ?>] transition-all group">
    ...
    <p class="text-3xl font-black text-gray-900 dark:text-white mb-1">96%</p>
...
<div class="bg-gradient-to-r from-blue-50 to-white dark:from-slate-800 dark:to-slate-800 border-2 border-blue-200 dark:border-slate-700 rounded-3xl p-6 shadow-sm transition-colors">
  <div class="flex items-start gap-4">
    <div class="w-12 h-12 rounded-xl bg-[<?= $color['warna_primary'] ?>] flex items-center justify-center flex-shrink-0 shadow-lg">
      <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewbox="0 0 24 24">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
      </svg>
    </div>
    <div class="flex-1">
      <h3 class="font-bold text-gray-900 dark:text-white text-lg mb-2"><?= lang('Admin/JadwalPelajaran.about_lock') ?></h3>
      <p class="text-sm font-medium text-gray-600 dark:text-slate-400 mb-4"><?= lang('Admin/JadwalPelajaran.about_lock_desc') ?></p>
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div class="flex items-start gap-3 bg-white dark:bg-slate-700/50 p-3 rounded-xl border border-gray-100 dark:border-slate-600 shadow-sm transition-colors">
          ...
          <div>
            <p class="text-sm font-bold text-gray-800 dark:text-slate-200"><?= lang('Admin/JadwalPelajaran.lock_feat_1') ?></p>
            <p class="text-xs text-gray-500 dark:text-slate-400 mt-0.5"><?= lang('Admin/JadwalPelajaran.lock_feat_1_desc') ?></p>
          </div>
        </div>
        <div class="flex items-start gap-3 bg-white dark:bg-slate-700/50 p-3 rounded-xl border border-gray-100 dark:border-slate-600 shadow-sm transition-colors">
          <svg class="w-5 h-5 text-emerald-600 dark:text-emerald-400 flex-shrink-0 mt-0.5" fill="none" stroke="currentColor" viewbox="0 0 24 24">
          ...
          <div>
            <p class="text-sm font-bold text-gray-800 dark:text-slate-200"><?= lang('Admin/JadwalPelajaran.lock_feat_2') ?></p>
            <p class="text-xs text-gray-500 dark:text-slate-400 mt-0.5"><?= lang('Admin/JadwalPelajaran.lock_feat_2_desc') ?></p>
          </div>
        </div>
        <div class="flex items-start gap-3 bg-white dark:bg-slate-700/50 p-3 rounded-xl border border-gray-100 dark:border-slate-600 shadow-sm transition-colors">
          ...
          <div>
            <p class="text-sm font-bold text-gray-800 dark:text-slate-200"><?= lang('Admin/JadwalPelajaran.lock_feat_3') ?></p>
            <p class="text-xs text-gray-500 dark:text-slate-400 mt-0.5"><?= lang('Admin/JadwalPelajaran.lock_feat_3_desc') ?></p>
          </div>
        </div>
      </div>
    </div>
  ...
  <div class="p-6 space-y-5 overflow-y-auto flex-1 custom-scrollbar">
    <div>
      ...
      <div class="flex items-center gap-3 p-4 bg-gray-50 dark:bg-slate-700/50 rounded-xl border border-gray-100 dark:border-slate-600 transition-colors">
        ...
        <div class="min-w-0">
          <p class="font-bold text-gray-800 dark:text-white truncate" id="drawerTeacher">Ahmad Budiman, S.Pd</p>
        </div>
      </div>
    </div>
    <div>
      ...
      <div class="p-4 bg-gray-50 dark:bg-slate-700/50 rounded-xl border border-gray-100 dark:border-slate-600 transition-colors">
        <p class="font-bold text-gray-800 dark:text-white" id="drawerRombel">VII-A</p>
      </div>
    </div>
  ...
  <div class="absolute inset-0 flex items-center justify-center p-4 pointer-events-none md:pl-64">
    <div class="relative w-full bg-white dark:bg-slate-800 rounded-3xl shadow-2xl flex flex-col max-h-[90vh] pointer-events-auto border border-transparent dark:border-slate-700 transition-colors duration-300" style="max-width: 600px;">
      ...
      <div class="p-6 border-b border-gray-100 dark:border-slate-700 flex items-center justify-between bg-white dark:bg-slate-800 rounded-t-3xl z-20 flex-shrink-0 transition-colors">
        ...
        <button type="button" onclick="closeCreateScheduleModal()" class="p-2 hover:bg-gray-100 dark:hover:bg-slate-700 rounded-full transition-colors cursor-pointer relative z-50 text-gray-500 dark:text-slate-400 outline-none">
      ...
      <div class="flex-1 overflow-y-auto p-6 relative z-10 custom-scrollbar">
        <form id="formTambahJadwal" class="space-y-5" onsubmit="handleCreateSchedule(event)">
          ...
          <div>
              ...
              <select name="guru_id" id="add_guru_id" required class="w-full px-4 py-3 bg-gray-50 dark:bg-slate-900 border border-gray-200 dark:border-slate-600 rounded-xl text-sm text-gray-800 dark:text-white focus:outline-none transition-colors appearance-none outline-none shadow-sm pointer-events-none opacity-70">
                  <option value="">Pilih Mapel Terlebih Dahulu</option>
              </select>
              <p class="text-xs text-gray-500 dark:text-slate-400 mt-1">*Guru ditentukan otomatis berdasarkan data Mapping Mapel.</p>
          </div>          
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <select name="hari" id="add_hari" required class="w-full px-4 py-3 bg-white dark:bg-slate-700 border border-gray-200 dark:border-slate-600 rounded-xl text-sm text-gray-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-[<?= $color['warna_primary'] ?>] transition-colors appearance-none cursor-pointer outline-none shadow-sm">
                ...
                <option value="Senin"><?= lang('Admin/JadwalPelajaran.day_monday') ?></option>
                <option value="Selasa"><?= lang('Admin/JadwalPelajaran.day_tuesday') ?></option>
                <option value="Rabu"><?= lang('Admin/JadwalPelajaran.day_wednesday') ?></option>
                <option value="Kamis"><?= lang('Admin/JadwalPelajaran.day_thursday') ?></option>
                <option value="Jumat"><?= lang('Admin/JadwalPelajaran.day_friday') ?></option>
              </select>
            </div>
            <div>
              ...
              <select name="jam_ke" id="add_jam_ke" required class="w-full px-4 py-3 bg-white dark:bg-slate-700 border border-gray-200 dark:border-slate-600 rounded-xl text-sm text-gray-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-[<?= $color['warna_primary'] ?>] transition-colors appearance-none cursor-pointer outline-none shadow-sm">
                <option value=""><?= lang('Admin/JadwalPelajaran.ph_period') ?></option>
                <option value="1">1 (07:30-08:10)</option>
                <option value="2">2 (08:10-08:50)</option>
                <option value="3">3 (08:50-09:30)</option>
                <option value="4">4 (09:45-10:25)</option>
                <option value="5">5 (10:25-11:05)</option>
                <option value="6">6 (11:05-11:45)</option>
                <option value="7">7 (13:00-13:40)</option>
                <option value="8">8 (13:40-14:20)</option>
              </select>
            </div>
          </div>
    ...
    <div class="relative w-full bg-white dark:bg-slate-800 rounded-3xl shadow-2xl flex flex-col max-h-[90vh] pointer-events-auto border border-transparent dark:border-slate-700 transition-colors duration-300" style="max-width: 600px;">
      <div class="p-6 border-b border-gray-100 dark:border-slate-700 flex items-center justify-between bg-white dark:bg-slate-800 rounded-t-3xl z-20 flex-shrink-0 transition-colors">
        <div>
          ...
          <p class="text-sm font-medium text-gray-500 dark:text-slate-400 mt-1"><?= lang('Admin/JadwalPelajaran.modal_lock_desc') ?></p>
        </div>
        <button onclick="closeLockScheduleModal()" class="p-2 hover:bg-gray-100 dark:hover:bg-slate-700 rounded-full transition-colors cursor-pointer relative z-50 text-gray-500 dark:text-slate-400 outline-none">
          <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" /></svg>
        </button>
      </div>
      <div class="flex-1 overflow-y-auto p-6 relative z-10 space-y-6 custom-scrollbar">
        <div class="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800/50 rounded-2xl p-6 shadow-sm transition-colors">
          <div class="flex items-start gap-4 mb-2">
            ...
            <div>
              ...
              <p class="text-sm font-medium text-blue-800 dark:text-blue-300">Tahun Ajaran 2024/2025 - Semester Ganjil</p>
            </div>
          </div>
        </div>
        ...
        <div class="flex gap-3 pt-5 border-t border-gray-100 dark:border-slate-700 sticky bottom-0 bg-white dark:bg-slate-800 z-50 pb-2 transition-colors">
          ...
          <button onclick="handleLockSchedule()" class="flex-1 px-6 py-3.5 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl transition-transform transform hover:-translate-y-0.5 shadow-lg shadow-blue-600/30 flex items-center justify-center gap-2 cursor-pointer relative outline-none">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewbox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" /></svg> <?= lang('Admin/JadwalPelajaran.btn_lock') ?>
          </button>
        </div>
      </div>
    </div>
...
<div id="importModal" class="fixed inset-0 hidden" style="z-index: 100000 !important;">
    ...
    <div class="absolute inset-0 flex items-center justify-center p-4 pointer-events-none md:pl-64">
        <div class="relative w-full bg-white dark:bg-slate-800 rounded-3xl shadow-2xl flex flex-col max-h-[90vh] pointer-events-auto border border-transparent dark:border-slate-700 transition-colors duration-300" style="max-width: 500px; z-index: 100001;">
            <div class="p-6 border-b border-gray-100 dark:border-slate-700 flex items-center justify-between bg-white dark:bg-slate-800 rounded-t-3xl z-20 flex-shrink-0 transition-colors">
                <div>
                    <h3 class="text-xl font-bold text-gray-800 dark:text-white"><?= lang('Admin/JadwalPelajaran.import_title') ?></h3>
                    <p class="text-sm font-medium text-gray-500 dark:text-slate-400 mt-1"><?= lang('Admin/JadwalPelajaran.import_desc') ?></p>
                </div>
            ...
            <div class="p-6">
                <form id="importForm" action="<?= base_url('admin/jadwal/import') ?>" method="POST" onsubmit="handleImportSubmit(event)" enctype="multipart/form-data">
                    <?= csrf_field() ?>
                    <div class="mb-5">
                        <p class="text-sm font-bold text-gray-700 dark:text-slate-300 mb-2"><?= lang('Admin/JadwalPelajaran.import_step_1') ?></p>
                        <a href="<?= base_url('admin/jadwal/template') ?>" class="inline-flex items-center gap-2 px-4 py-2 font-bold rounded-lg transition-colors text-sm shadow-sm hover:opacity-80" style="color: <?= $color['warna_primary'] ?>; background-color: <?= $color['warna_primary'] ?>1A;">
                            ...
                            <?= lang('Admin/JadwalPelajaran.import_dl_temp') ?>
                        </a>
                    </div>
                    <div class="mb-6">
                        <p class="text-sm font-bold text-gray-700 dark:text-slate-300 mb-2"><?= lang('Admin/JadwalPelajaran.import_step_2') ?></p>
                    ...
                    <div class="flex justify-end gap-3 pt-4 border-t border-gray-100 dark:border-slate-700 transition-colors">
                        <button type="button" onclick="closeImportModal()" class="px-5 py-2.5 bg-white dark:bg-slate-700 border border-gray-300 dark:border-slate-600 text-gray-700 dark:text-slate-300 font-bold rounded-xl hover:bg-gray-50 dark:hover:bg-slate-600 transition-colors shadow-sm outline-none"><?= lang('Admin/JadwalPelajaran.btn_cancel') ?></button>
                        <button type="submit" class="px-5 py-2.5 text-white font-bold rounded-xl shadow-lg transition-transform transform hover:-translate-y-0.5 flex items-center gap-2 outline-none cursor-pointer" style="background-color: <?= $color['warna_primary'] ?>; box-shadow: 0 10px 15px -3px <?= $color['warna_primary'] ?>40;">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12" /></svg> <?= lang('Admin/JadwalPelajaran.btn_upload_import') ?>
                        </button>
                    </div>
                </form>
            </div>
...
<script>
    ...
    window.LANG = {
        ...
        js_feat_na: "<?= lang('Admin/JadwalPelajaran.js_feat_na') ?: 'Fitur Belum Tersedia' ?>",
        js_feat_na_desc: "<?= lang('Admin/JadwalPelajaran.js_feat_na_desc') ?: 'Sistem sedang dalam tahap pengembangan.' ?>",
        ...
        js_err_fatal: "<?= lang('Admin/JadwalPelajaran.js_err_fatal') ?: 'Terjadi kesalahan fatal server.' ?>",
        js_err_conn: "<?= lang('Admin/JadwalPelajaran.js_err_conn') ?: 'Koneksi terputus.' ?>"
    };

    // --- LOGIKA MAPPING DI MODAL TAMBAH JADWAL ---

...
<div class="warning-banner bg-amber-50 dark:!bg-amber-950/30 border-2 border-amber-300 dark:!border-amber-900/50 rounded-2xl p-4 mb-6 flex items-start gap-4 transition-colors shadow-sm" id="warningBanner">
    <svg class="w-8 h-8 text-amber-600 dark:!text-amber-500 flex-shrink-0" fill="none" stroke="currentColor" viewbox="0 0 24 24">
...
<div class="config-card bg-white dark:!bg-slate-800 border-2 border-gray-100 dark:!border-slate-700 rounded-3xl p-6 mb-6 shadow-sm transition-colors">
    ...
    <div class="grid grid-cols-1 md:grid-cols-5 gap-6">
        <div>
            <label class="block text-[11px] font-black text-gray-500 dark:!text-slate-400 mb-2 uppercase tracking-widest transition-colors">Tahun Ajaran</label>
            <select id="tahunAjaran" class="w-full px-4 py-3.5 bg-gray-50 dark:!bg-slate-700 border-2 border-gray-200 dark:!border-slate-600 rounded-xl text-gray-900 dark:!text-white focus:outline-none focus:border-[<?= $color['warna_primary'] ?>] transition-all cursor-pointer shadow-sm outline-none appearance-none" onchange="checkReadyToInput()">
                <?php foreach ($tahun_ajaran as $ta): ?>
                    <option value="<?= $ta['id'] ?>" <?= $ta['id'] == $active_ta_id ? 'selected' : '' ?>>
                        <?= esc($ta['tahun']) ?> (<?= esc($ta['semester']) ?>) <?= $ta['status'] == 'Aktif' ? ' - AKTIF' : '' ?>
                    </option>
                <?php endforeach; ?>
            </select>
        </div>
        ...
        <div>
            ...
            <input type="text" id="bobot" class="w-full px-4 py-3.5 bg-gray-50 dark:!bg-slate-700 border-2 border-gray-200 dark:!border-slate-600 rounded-xl text-gray-900 dark:!text-white font-bold focus:outline-none transition-all shadow-sm outline-none cursor-default" placeholder="0%" readonly>
        </div>
        ...
        <div>
            ...
            <input type="number" id="kkm" class="w-full px-4 py-3.5 bg-gray-50 dark:!bg-slate-700 border-2 border-gray-200 dark:!border-slate-600 rounded-xl text-gray-900 dark:!text-white font-bold focus:outline-none transition-all shadow-sm outline-none cursor-default" value="<?= esc($info['kkm']) ?>" readonly>
        </div>

        <div class="flex items-end">
            <button id="btnLoadData"
                data-url="<?= base_url('guru/nilai-sumatif/getNilaiSiswa') ?>"
    ...
    <div class="mt-5 p-4 bg-blue-50 dark:!bg-blue-900/20 border-l-4 border-blue-500 rounded-r-xl transition-colors">
        <p class="text-xs text-blue-800 dark:!text-blue-300 font-bold leading-relaxed">
            <svg class="w-4 h-4 inline mr-1.5 -mt-0.5" fill="currentColor" viewbox="0 0 20 20">
...
<div id="emptyState" class="bg-white dark:!bg-slate-800 rounded-3xl py-20 px-6 border-2 border-dashed border-gray-200 dark:!border-slate-700 text-center transition-colors shadow-sm">
    <svg class="w-40 h-40 text-gray-300 dark:!text-slate-700 mx-auto mb-6 opacity-60 transition-colors" fill="none" stroke="currentColor" viewbox="0 0 24 24">
...
<div id="actionToolbar" class="fixed bottom-6 left-1/2 transform -translate-x-1/2 w-[90%] md:w-[85%] lg:w-[calc(100%-20rem)] lg:ml-32 bg-white/90 dark:!bg-slate-800/90 backdrop-blur-md rounded-2xl shadow-2xl p-4 border border-gray-200 dark:!border-slate-700 transition-all z-40" style="display: none;">
    <div class="flex items-center justify-between gap-4 flex-wrap">
        <div class="flex items-center gap-3 bg-emerald-50 dark:!bg-emerald-900/20 px-4 py-2 rounded-xl transition-colors">
            <svg class="w-5 h-5 text-emerald-600 dark:!text-emerald-400 animate-pulse" fill="none" stroke="currentColor" viewbox="0 0 24 24">
...
<div id="confirmModal" class="fixed inset-0 z-[99999] hidden items-center justify-center p-4 bg-gray-950/80 backdrop-blur-sm transition-all overflow-y-auto" onclick="if(event.target === this) closeConfirmModal()">

    <div class="relative w-full max-w-md bg-white dark:!bg-slate-900 rounded-3xl shadow-2xl transition-colors border border-transparent dark:!border-slate-800 overflow-hidden">
...
<script>
    ...
    const SETTING_BOBOT = <?= json_encode($bobot_list ?? []) ?>;

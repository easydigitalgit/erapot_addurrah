...
<?= $this->section('title') ?>
<?= lang('WaliKelas/AkunSaya.page_title') ?> - <?= session()->get('nama_lengkap') ?>
<?= $this->endSection() ?>
...
<link rel="stylesheet" href="<?= base_url('assets/css/WaliKelas/akun-saya.css') ?>">
<?= $this->endSection() ?>

<?= $this->section('content') ?>
<div class="mb-6">
    <div class="flex items-center gap-2 text-sm text-gray-500 dark:text-slate-400 mb-3">
        <span class="text-[var(--warna-primary)] font-medium"><?= lang('WaliKelas/AkunSaya.page_title') ?></span>
    </div>
    ...
    <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 mb-6">
        <div>
            <h1 class="text-2xl md:text-3xl font-bold text-gray-900 dark:text-white mb-2 flex items-center gap-3">
                ...
                <?= lang('WaliKelas/AkunSaya.page_title') ?>
            </h1>
            <p class="text-sm md:text-base text-gray-600 dark:text-slate-300"><?= lang('WaliKelas/AkunSaya.page_subtitle') ?></p>
        </div>
    </div>
</div>
...
<div class="mb-6 rounded-2xl shadow-lg overflow-hidden border-0">
    <div class="bg-[var(--warna-primary)] p-6 md:p-8 text-white relative">
        ...
        <div class="flex flex-col md:flex-row gap-6 items-center md:items-start relative z-10">
            <?php
            $namaLengkap = session()->get('nama_lengkap') ?? $user['nama_lengkap'] ?? 'Wali Kelas';
            ...
            if ($fotoProfil && file_exists(FCPATH . 'assets/uploads/avatars/' . $fotoProfil)) {
                ...
                $avatarUrl = "https://ui-avatars.com/api/?name={$inisial}&background=ffffff&color=3b82f6&size=160&bold=true&rounded=true";
            }
            ?>
                ...
                <div class="flex flex-col md:flex-row md:items-start justify-between mb-6 gap-4">
                    ...
                    <div class="flex flex-wrap items-center justify-center md:justify-end gap-2 mt-2 md:mt-0">
                        <span class="inline-flex items-center gap-2 px-3 py-1.5 bg-emerald-500/20 backdrop-blur-md border border-emerald-400/30 text-emerald-50 rounded-full text-[10px] font-black uppercase tracking-widest shadow-sm">
                            ...
                            <?= lang('WaliKelas/AkunSaya.status_active') ?? 'AKTIF' ?>
                        </span>

                        <?php if (!empty($semua_role)): ?>
                            <?php foreach ($semua_role as $role): ?>
                                <span class="inline-flex items-center px-3 py-1.5 bg-white/10 backdrop-blur-md border border-white/20 text-white rounded-full text-[10px] font-black uppercase tracking-widest shadow-sm hover:bg-white/20 transition-colors cursor-default">
                        ...
                        <?php else: ?>
                            <span class="inline-flex items-center px-3 py-1.5 bg-white/10 backdrop-blur-md border border-white/20 text-white rounded-full text-[10px] font-black uppercase tracking-widest shadow-sm cursor-default">
                                ...
                                <?= session()->get('role_label') ?? 'Wali Kelas' ?>
                            </span>
                        <?php endif; ?>
                    </div>
                </div>
...
<div class="card mb-6 bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700">
    <h3 class="text-xl font-bold text-gray-900 dark:text-white mb-6 flex items-center gap-2">
        ...
        <?= lang('WaliKelas/AkunSaya.personal_title') ?? 'Informasi Pribadi' ?>
    </h3>
    ...
    <form id="personalInfoForm">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            <div>
                <label class="block text-sm font-semibold text-gray-700 dark:text-slate-300 mb-2"><?= lang('WaliKelas/AkunSaya.full_name') ?? 'Nama Lengkap' ?> <span class="text-red-500">*</span></label>
            ...
            <div>
                <label class="block text-sm font-semibold text-gray-700 dark:text-slate-300 mb-2"><?= lang('WaliKelas/AkunSaya.email') ?? 'Email' ?> <span class="text-red-500">*</span></label>
            ...
            <div>
                <label class="block text-sm font-semibold text-gray-700 dark:text-slate-300 mb-2"><?= lang('WaliKelas/AkunSaya.phone') ?? 'No. Telepon' ?></label>
            ...
            <div class="md:col-span-2">
                <label class="block text-sm font-semibold text-gray-700 dark:text-slate-300 mb-2"><?= lang('WaliKelas/AkunSaya.address') ?? 'Alamat' ?></label>
        ...
        <div class="flex justify-end mt-6">
            <button type="submit" id="btnSaveInfo" class="bg-[var(--warna-primary)] px-6 py-2.5 text-white font-bold rounded-xl hover:brightness-90 transition-all flex items-center gap-2 shadow-md">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
                </svg>
                <span id="textSaveInfo"><?= lang('WaliKelas/AkunSaya.btn_save_changes') ?? 'Simpan Perubahan' ?></span>
            </button>
        </div>
    </form>
</div>
...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
    ...
    <div class="card bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700">
        <h3 class="text-xl font-bold text-gray-900 dark:text-white mb-6 flex items-center gap-2">
            ...
            <?= lang('WaliKelas/AkunSaya.account_sec') ?? 'Keamanan Akun' ?>
        </h3>

        <div class="p-4 bg-gray-50 dark:bg-slate-700 rounded-xl border border-gray-100 dark:border-slate-600 mb-4">
            <div class="flex items-center justify-between mb-2">
                <p class="font-semibold text-gray-900 dark:text-white"><?= lang('WaliKelas/AkunSaya.password') ?? 'Kata Sandi' ?></p>
                <button onclick="showChangePasswordModal()" class="text-sm text-white bg-[var(--warna-primary)] px-3 py-1.5 rounded-lg hover:brightness-90 font-bold transition-all shadow-sm">
                    <?= lang('WaliKelas/AkunSaya.btn_change_pass') ?? 'Ubah Sandi' ?>
                </button>
            </div>
            <p class="text-sm text-gray-600 dark:text-slate-300"><?= lang('WaliKelas/AkunSaya.sec_desc') ?? 'Perbarui kata sandi secara berkala untuk menjaga keamanan akun Anda.' ?></p>
        </div>
        ...
        <div class="p-4 bg-[var(--warna-secondary)] border border-[var(--warna-primary)] rounded-xl">
            <div class="flex items-start gap-2">
                ...
                <div>
                    <p class="text-sm text-[var(--warna-primary)] font-semibold mb-1"><?= lang('WaliKelas/AkunSaya.sec_tips') ?? 'Tips Keamanan' ?></p>
                    <ul class="text-sm text-[var(--warna-primary)] space-y-1">
                        <li>• <?= lang('WaliKelas/AkunSaya.tip_1') ?? 'Gunakan kombinasi huruf, angka, dan simbol.' ?></li>
                        <li>• <?= lang('WaliKelas/AkunSaya.tip_2') ?? 'Jangan beritahukan kata sandi Anda kepada siapapun.' ?></li>
                    </ul>
                </div>
            </div>
        </div>
    ...
    <div class="card bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700">
        <h3 class="text-xl font-bold text-gray-900 dark:text-white mb-6 flex items-center gap-2">
            ...
            <?= lang('WaliKelas/AkunSaya.sys_pref') ?? 'Preferensi Sistem' ?>
        </h3>
        ...
        <div class="space-y-5">
            <div>
                <label class="block text-sm font-semibold text-gray-700 dark:text-slate-300 mb-2"><?= lang('WaliKelas/AkunSaya.sys_lang') ?? 'Bahasa Sistem' ?></label>
                <select id="pref_bahasa" class="w-full border border-gray-200 dark:border-slate-600 bg-gray-50 dark:bg-slate-700 text-gray-900 dark:text-white rounded-xl p-3 focus:border-[var(--warna-primary)] outline-none transition-colors">
                    <option value="id" <?= $currentLang == 'id' ? 'selected' : '' ?>><?= lang('WaliKelas/AkunSaya.lang_id') ?? 'Indonesia' ?></option>
                    <option value="en" <?= $currentLang == 'en' ? 'selected' : '' ?>><?= lang('WaliKelas/AkunSaya.lang_en') ?? 'English' ?></option>
                    <option value="ar" <?= $currentLang == 'ar' ? 'selected' : '' ?>><?= lang('WaliKelas/AkunSaya.lang_ar') ?? 'العربية (Arabic)' ?></option>
                </select>
            </div>

            <div>
                <label class="block text-sm font-semibold text-gray-700 dark:text-slate-300 mb-3"><?= lang('WaliKelas/AkunSaya.display_mode') ?? 'Mode Tampilan' ?></label>
                <div class="space-y-3">
                    <label class="flex items-center gap-3 p-3 rounded-xl border border-gray-200 dark:border-slate-600 hover:bg-gray-50 dark:hover:bg-slate-700 cursor-pointer transition-colors bg-white dark:bg-slate-800">
                        ...
                        <div class="flex items-center gap-2 flex-1">
                            ...
                            <div>
                                <p class="font-semibold text-gray-900 dark:text-white text-sm"><?= lang('WaliKelas/AkunSaya.light_mode') ?? 'Mode Terang' ?></p>
                                <p class="text-xs text-gray-500 dark:text-slate-400"><?= lang('WaliKelas/AkunSaya.light_desc') ?? 'Tampilan standar untuk siang hari.' ?></p>
                            </div>
                        </div>
                    </label>
                    ...
                    <label class="flex items-center gap-3 p-3 rounded-xl border border-gray-200 dark:border-slate-600 hover:bg-gray-50 dark:hover:bg-slate-700 cursor-pointer transition-colors bg-white dark:bg-slate-800">
                        ...
                        <div class="flex items-center gap-2 flex-1">
                            ...
                            <div>
                                <p class="font-semibold text-gray-900 dark:text-white text-sm"><?= lang('WaliKelas/AkunSaya.dark_mode') ?? 'Mode Gelap' ?></p>
                                <p class="text-xs text-gray-500 dark:text-slate-400"><?= lang('WaliKelas/AkunSaya.dark_desc') ?? 'Tampilan nyaman untuk mata di malam hari.' ?></p>
                            </div>
                        </div>
                    </label>
                </div>
            ...
            <div class="flex justify-end pt-4">
                <button id="btnSavePref" onclick="savePreferences()" class="bg-[var(--warna-primary)] px-6 py-2.5 text-white font-bold rounded-xl hover:brightness-90 transition-all flex items-center gap-2 shadow-md">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
                    </svg>
                    <span id="textSavePref"><?= lang('WaliKelas/AkunSaya.btn_save_pref') ?? 'Simpan Preferensi' ?></span>
                </button>
            </div>
        </div>
    </div>
...
<div class="card mb-6 bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700">
    <h3 class="text-xl font-bold text-gray-900 dark:text-white mb-6 flex items-center gap-2">
        ...
        <?= lang('WaliKelas/AkunSaya.recent_login') ?? 'Riwayat Login Terakhir' ?>
    </h3>
    <div class="overflow-x-auto rounded-xl border border-gray-100 dark:border-slate-700">
        <table class="w-full text-left">
            <thead class="bg-gray-50 dark:bg-slate-700/50 border-b border-gray-100 dark:border-slate-700">
                <tr>
                    <th class="py-3 px-4 text-sm font-semibold text-gray-600 dark:text-slate-300"><?= lang('WaliKelas/AkunSaya.th_time') ?? 'Waktu' ?></th>
                    <th class="py-3 px-4 text-sm font-semibold text-gray-600 dark:text-slate-300"><?= lang('WaliKelas/AkunSaya.th_device') ?? 'Perangkat' ?></th>
                    <th class="py-3 px-4 text-sm font-semibold text-gray-600 dark:text-slate-300"><?= lang('WaliKelas/AkunSaya.th_ip') ?? 'Alamat IP' ?></th>
                    <th class="py-3 px-4 text-sm font-semibold text-gray-600 dark:text-slate-300"><?= lang('WaliKelas/AkunSaya.th_status') ?? 'Status' ?></th>
                </tr>
            </thead>
            <tbody class="divide-y divide-gray-100 dark:divide-slate-700">
                <?php if (empty($login_logs)): ?>
                    <tr>
                        <td colspan="4" class="text-center py-6 text-gray-500 dark:text-slate-400"><?= lang('WaliKelas/AkunSaya.empty_login') ?? 'Belum ada riwayat login.' ?></td>
                    </tr>
                <?php else: ?>
                    <?php foreach ($login_logs as $log): ?>
                        <tr class="hover:bg-gray-50 dark:hover:bg-slate-700/50 transition">
                            ...
                            <td class="py-3 px-4 font-mono text-sm text-gray-700 dark:text-slate-300"><?= htmlspecialchars($log['ip_address'] ?? 'Unknown') ?></td>
                            <td class="py-3 px-4">
...
<div id="passwordModal" class="fixed inset-0 bg-black/60 backdrop-blur-sm z-[100] flex items-center justify-center opacity-0 pointer-events-none transition-opacity duration-300">
    <div class="bg-white dark:bg-slate-800 rounded-3xl w-full max-w-md p-6 transform scale-95 transition-transform duration-300 shadow-2xl border border-gray-100 dark:border-slate-700">
        <div class="text-center mb-6">
            ...
            <h3 class="text-xl font-black text-gray-900 dark:text-white mb-2"><?= lang('WaliKelas/AkunSaya.modal_pass_title') ?? 'Ubah Kata Sandi' ?></h3>
        </div>
        ...
        <form id="changePasswordForm">
            ...
            <div class="flex gap-3">
                <button type="button" onclick="closePasswordModal()" class="flex-1 py-3 px-4 bg-gray-100 dark:bg-slate-700 text-gray-700 dark:text-slate-300 font-bold rounded-xl hover:bg-gray-200 dark:hover:bg-slate-600 transition-colors"><?= lang('WaliKelas/AkunSaya.btn_cancel') ?? 'Batal' ?></button>
                <button type="submit" id="btnSavePassword" class="flex-1 py-3 px-4 bg-[var(--warna-primary)] text-white font-bold rounded-xl hover:brightness-90 transition-all shadow-md">
                    <span id="textSavePassword"><?= lang('WaliKelas/AkunSaya.btn_change_pass') ?? 'Ubah Sandi' ?></span>
                </button>
            </div>
        </form>
    </div>
...
<script>
    ...
    const LANG = {
        saving: "<?= lang('WaliKelas/AkunSaya.js_saving') ?? 'Menyimpan...' ?>",
        success: "<?= lang('WaliKelas/AkunSaya.js_success') ?? 'Berhasil!' ?>",
        failed: "<?= lang('WaliKelas/AkunSaya.js_failed') ?? 'Gagal!' ?>",
        server_error: "<?= lang('WaliKelas/AkunSaya.js_server_error') ?? 'Terjadi kesalahan server.' ?>",
        err_pwd_match: "<?= lang('WaliKelas/AkunSaya.js_err_pwd_match') ?? 'Kata sandi tidak cocok.' ?>",
        err_pwd_len: "<?= lang('WaliKelas/AkunSaya.js_err_pwd_len') ?? 'Kata sandi minimal 8 karakter.' ?>",
        err_img_size: "<?= lang('WaliKelas/AkunSaya.js_err_img_size') ?? 'Ukuran gambar terlalu besar.' ?>",
        uploading: "<?= lang('WaliKelas/AkunSaya.js_uploading') ?? 'Mengunggah...' ?>",
        photo_updated: "<?= lang('WaliKelas/AkunSaya.js_photo_updated') ?? 'Foto diperbarui.' ?>",
        btn_change_pass: "<?= lang('WaliKelas/AkunSaya.btn_change_pass') ?? 'Ubah Sandi' ?>",
        btn_save_changes: "<?= lang('WaliKelas/AkunSaya.btn_save_changes') ?? 'Simpan Perubahan' ?>"
    };
</script>
<script src="<?= base_url('assets/js/WaliKelas/akun-saya.js') ?>?v=<?= time() ?>"></script>
<?= $this->endSection() ?>
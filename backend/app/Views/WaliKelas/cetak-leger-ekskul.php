    ...
    <div class="lg:col-span-3 bg-white dark:bg-slate-800 p-6 rounded-3xl border border-gray-100 dark:border-slate-700 shadow-sm">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
            ...
            <div>
                ...
                <select id="filter_rombel" class="w-full px-4 py-3 bg-gray-100 dark:bg-slate-700 border border-gray-200 dark:border-slate-600 text-gray-500 dark:text-slate-400 rounded-xl cursor-not-allowed" disabled>
                    <?php if ($rombel): ?>
                        <option value="<?= $rombel['id'] ?>" selected><?= $rombel['nama_rombel'] ?></option>
                    <?php else: ?>
                        <option value=""><?= lang('Admin/LegerEkskul.js_no_data') ?></option>
                    <?php endif; ?>
                </select>
            </div>
...
<script src="<?= base_url('assets/js/WaliKelas/cetak-leger-ekskul.js') ?>?v=<?= time() ?>"></script>
<?= $this->endSection() ?>

     ...
     <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6 no-print w-full min-w-0">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-5 mb-5">
         ...
         <div>
             ...
             <select id="filter_rombel" class="select-custom w-full px-4 py-3 bg-gray-100 dark:bg-slate-700 border border-gray-200 dark:border-slate-600 text-gray-500 dark:text-slate-400 rounded-xl transition-colors appearance-none outline-none shadow-sm cursor-not-allowed" disabled> 
                <?php if ($rombel): ?>
                    <option value="<?= $rombel['id'] ?>" selected><?= $rombel['nama_rombel'] ?></option>
                <?php else: ?>
                    <option value=""><?= lang('Admin/CetakLeger.js_empty_data') ?></option>
                <?php endif; ?>
             </select>
         </div>
        </div>
...
<script src="<?= base_url('assets/js/WaliKelas/cetak-leger.js') ?>?v=<?= time() ?>"></script>
<?= $this->endSection() ?>
